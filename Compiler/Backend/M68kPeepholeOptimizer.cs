...
internal sealed partial class M68kPeepholeOptimizer : IM68kOptimizerPass
{
	...
	private AddressEntryBoundaryIndex? _addressEntryBoundaryIndex;
	...
	public void Run()
	{
		...
		do
		{
			...
			try
			{
				_addressEntryBoundaryIndex = null;
				...
				var groupedTransfer = _rewriteBudget == int.MaxValue &&
					(TryGroupContiguousLongMemoryPush(dataflow) ||
					 TryGroupContiguousLongMemoryCopy(dataflow));
				if (!groupedTransfer && _rewriteBudget == int.MaxValue)
				{
					appliedRewrites = ApplyIndependentLocalBatch(
						dataflow.Instructions);
					if (appliedRewrites == 0)
					{
						appliedRewrites = ApplyMemoryCopyBatch(dataflow);
					}
				}
				changed = groupedTransfer || appliedRewrites != 0 ||
					(!_assembler.HasAnalysisScope &&
						(TryInlineBareReturnBranches() ||
						 TryBypassEmptySwitchEdges() ||
						 TryFoldTailReturn() ||
						 TryLayoutColdTerminalBranch() ||
						 TryRemoveBranchToNextLabel())) ||
				TryFoldBooleanComparisonInversion(dataflow) ||
				TryThreadConstantBooleanEdge(dataflow) ||
				...
				TryRemoveExactSameRegisterFrameReload() ||
				...
				TryCleanupFrameMemoryTransfers(dataflow) ||
				TryForwardStackStoreReload(dataflow) ||
				TryReplaceZeroAddressMove() ||
				TryFoldZeroMoveQuickToAddress(dataflow) ||
				TryFoldSmallAbsoluteAddressLoad(dataflow) ||
				TryOptimizeSmallAddressImmediate(dataflow) ||
				TryFoldConstantAddressDisplacement(dataflow) ||
				...
				TryFoldLongImmediateIntoSingleUse(dataflow) ||
				TryReplaceStackImmediateWithPea(dataflow) ||
				TryReplaceLeaStackPushWithPea(dataflow) ||
				...
				TryRemoveRedundantZeroBeforeWordMaskStore(dataflow) ||
				TryRetargetZeroExtendedWordLoad(dataflow) ||
				...
				TryFoldLeaToAddressRegisterMove(dataflow) ||
				...
				TryForwardAddressRegisterMoveIntoCompare(dataflow) ||
				TryForwardMemoryLoadToAddressPair(dataflow) ||
				...
				TryForwardRegisterCopyToMemory(dataflow) ||
				(_rewriteBudget != int.MaxValue &&
					TryFoldMemoryCopyThroughDeadDataRegister(dataflow)) ||
				TryRemoveCallToRtsLeaf() ||
				TryFoldLeafMemoryReadCall() ||
				TryFoldLeafMemoryReadCallWithLateAddress() ||
				TryFoldLeafMemoryWriteCall(dataflow) ||
				...
				TryNarrowZeroExtendedRegisterAnd(dataflow) ||
				...
				TryRemoveUnusedRegisterStackPreservation() ||
				TryFoldAdjacentStackReleases() ||
		...
		IReadOnlyList<M68kEmittedInstruction> instructions)
	{
		...
		foreach (var instruction in instructions)
		{
			...
			var sizeCode = (instruction.Opcode >> 12) & 0x0F;
			if (sizeCode is 1 or 2 or 3)
			{
				if (((instruction.Opcode >> 3) & 7) == 5 &&
					_buffer.ReadWord(instruction.Offset + 2) == 0)
				{
					candidates.Add((instruction, 4));
				}
				else if (((instruction.Opcode >> 6) & 7) == 5 &&
					_buffer.ReadWord(
						instruction.Offset + instruction.Length - 2) == 0)
				{
					candidates.Add((instruction, 5));
				}
			}
			else if (((instruction.Opcode >> 3) & 7) == 5 &&
				_buffer.ReadWord(
					instruction.Offset + instruction.Length - 2) == 0)
			{
				candidates.Add((instruction, 6));
			}
		}

		var removals = new List<(int Offset, int Count)>(candidates.Count);
		for (var index = candidates.Count - 1; index >= 0; index--)
		{
			...
			if (kind == 1)
			{
				...
				removals.Add((instruction.Offset + 2, 4));
				continue;
			}
			if (kind <= 3)
			{
				...
				removals.Add((instruction.Offset + 2, kind == 2 ? 4 : 2));
				continue;
			}

			var source = kind is 4 or 6;
			_buffer.WriteWord(
				instruction.Offset,
				source
					? (ushort)((instruction.Opcode & 0xFFC7) | 0x0010)
					: (ushort)((instruction.Opcode & 0xFE3F) | 0x0080));
			removals.Add((
				source
					? instruction.Offset + (kind == 4 ? 2 : instruction.Length - 2)
					: instruction.Offset + instruction.Length - 2,
				2));
		}
	...
	private int ApplyMemoryCopyBatch(M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		var candidates = new List<(M68kEmittedInstruction Load,
		...
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			{
				candidates.Add((load, store));
			}
		}

		foreach (var candidate in candidates)
		{
			_buffer.WriteWord(candidate.Load.Offset, (ushort)(
				(candidate.Store.Opcode & 0xFFC0) |
				(candidate.Load.Opcode & 0x003F)));
		}
		_buffer.RemoveByteRanges(candidates
			.Select(static candidate => (candidate.Store.Offset, 2))
			.ToArray());
		return candidates.Count;
		...
		do
		{
			...
			try
			{
				...
				_addressFixupOffsets = _buffer.Addresses
					.Select(static address => address.Offset)
					.ToHashSet();
				changed = TryInlineBareReturnBranches() ||
					TryBypassEmptySwitchEdges() ||
					TryFoldTailReturn() ||
					TryLayoutColdTerminalBranch() ||
					TryRemoveBranchToNextLabel();
	...
	private bool TryFoldSmallAbsoluteAddressLoad(M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			if ((address.Opcode & 0xF1FF) != 0x207C || address.Length != 6 ||
				HasAddressFixup(address) || IsReferencedLabelAt(address.Offset) ||
				(load.Opcode & 0xF1F8) != 0x2010 || load.Length != 2 ||
				IsReferencedLabelAt(load.Offset) ||
				((address.Opcode >> 9) & 7) != (load.Opcode & 7) ||
				!IsSignedWord(address.ExtensionLong) ||
				!dataflow.TryGetFacts(load.Offset, out var facts) ||
				(facts.LiveAddressAfter & (1 << (address.Opcode >> 9 & 7))) != 0)
			{
				continue;
			}

			var destination = (load.Opcode >> 9) & 7;
			_buffer.WriteWord(address.Offset,
				(ushort)(0x2038 | (destination << 9))); // MOVE.L abs.w,Dn
			_buffer.WriteWord(address.Offset + 2, unchecked((short)address.ExtensionLong));
			_buffer.RemoveBytes(address.Offset + 4, 4);
			return true;
		}
		return false;
	}
	...
	private bool TryReplaceLeaStackPushWithPea(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			if ((lea.Opcode & 0xF1C0) != 0x41C0 || // LEA <ea>,An
				lea.Length is not (2 or 4 or 6) ||
				(push.Opcode & 0xFFF8) != 0x2F08 || // MOVE.L An,-(A7)
				push.Length != 2 ||
				HasLabelAt(push.Offset) ||
			...
			var temporary = (lea.Opcode >> 9) & 7;
			if (temporary == 7 ||
				(push.Opcode & 7) != temporary ||
				(pushFacts.LiveAddressAfter & (1 << temporary)) != 0 ||
				(pushFacts.Effects.WritesConditions &
				 pushFacts.LiveConditionsAfter) != M68kConditionCodeSet.None)
			{
				continue;
			}

			// PEA accepts the same control addressing modes as LEA. Reusing the
			// effective-address bits and extension words keeps stack-relative and
			// PC-relative address calculation anchored at the original instruction.
			_buffer.WriteWord(lea.Offset,
				(ushort)(0x4840 | (lea.Opcode & 0x003F)));
	...
	private bool TryFoldLongImmediateIntoSingleUse(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			var use = instructions[index + 1];
			if ((immediate.Opcode & 0xF1FF) != 0x203C ||
				immediate.Length != 6 ||
				HasLabelAt(use.Offset) ||
				!dataflow.TryGetFacts(use.Offset, out var useFacts))
			{
				continue;
			}

			var temporary = (immediate.Opcode >> 9) & 7;
			if ((useFacts.LiveDataAfter & (1 << temporary)) != 0)
			{
				continue;
			}

			var value = unchecked((int)immediate.ExtensionLong);
			if (value is >= sbyte.MinValue and <= sbyte.MaxValue)
			{
				// MOVEQ plus a register operation is no larger, and is often smaller.
				continue;
			}

			if (TryGetImmediateDataOperation(
					use.Opcode,
					temporary,
					out var replacement,
				...
				use.Length == 2)
			{
				if (operandBytes != 4 && HasAddressFixup(immediate))
				{
					continue;
				}

				_buffer.WriteWord(immediate.Offset, replacement);
				if (operandBytes == 4)
				{
				...
				else
				{
					...
					_buffer.RemoveBytes(immediate.Offset + 4, 4);
				}
				return true;
			}

			var sizeCode = (use.Opcode >> 12) & 0x0F;
			var destinationMode = (use.Opcode >> 6) & 7;
			var destinationRegister = (use.Opcode >> 9) & 7;
			if (sizeCode is not (1 or 2 or 3) ||
				((use.Opcode >> 3) & 7) != 0 ||
				(use.Opcode & 7) != temporary ||
				!IsAlterableMemoryDestination(
					destinationMode,
					destinationRegister) ||
				DestinationUsesTemporaryAsDataIndex(
					use,
					destinationMode,
					temporary))
			{
			...
			var directMove = (ushort)((use.Opcode & 0xFFC0) | 0x003C);
			if (sizeCode == 2)
			{
				// MOVE.L #imm,<ea> keeps the immediate and destination extensions at
				// their existing offsets after the now-redundant store opcode is removed.
				_buffer.WriteWord(immediate.Offset, directMove);
				_buffer.RemoveBytes(use.Offset, 2);
				return true;
			}

			if (destinationMode == 7 ||
				HasAddressFixup(immediate) ||
				HasRelocationInRange(use.Offset + 2, use.Offset + use.Length))
			{
				continue;
			}

			var extensionWords = new ushort[(use.Length - 2) / 2];
			for (var extension = 0; extension < extensionWords.Length; extension++)
			{
				extensionWords[extension] =
					_buffer.ReadWord(use.Offset + 2 + extension * 2);
			}
			_buffer.WriteWord(immediate.Offset, directMove);
			_buffer.WriteWord(
				immediate.Offset + 2,
				unchecked((ushort)value));
			var replacementLength = 4;
			foreach (var extensionWord in extensionWords)
			{
				_buffer.WriteWord(immediate.Offset + replacementLength, extensionWord);
				replacementLength += 2;
			}
			_buffer.RemoveBytes(immediate.Offset + replacementLength, 4);
			return true;
		}

		return false;

		bool HasRelocationInRange(int startOffset, int endOffset) =>
			_buffer.Addresses.Any(fixup =>
				fixup.Offset >= startOffset && fixup.Offset < endOffset) ||
			_buffer.PcRelative.Any(fixup =>
				fixup.DisplacementOffset >= startOffset &&
				fixup.DisplacementOffset < endOffset);

		static bool DestinationUsesTemporaryAsDataIndex(
			M68kEmittedInstruction instruction,
			int mode,
			int register) =>
			mode == 6 &&
		...
		static bool TryGetImmediateDataOperation(
			ushort opcode,
			int temporary,
			out ushort replacement,
			out int operandBytes)
		{
			var operation = opcode & 0xF1F8;
			var eor = operation is 0xB100 or 0xB140 or 0xB180;
			var source = eor ? (opcode >> 9) & 7 : opcode & 7;
			var destination = eor ? opcode & 7 : (opcode >> 9) & 7;
			...
			var immediateOpcode = operation switch
			{
				0xB040 => 0x0C40, // CMP.W
				0xB080 => 0x0C80, // CMP.L
				0xC040 => 0x0240, // AND.W
			...
			replacement = (ushort)(immediateOpcode | destination);
			operandBytes = (operation & 0x00C0) switch
			{
				0x0000 => 1,
				0x0040 => 2,
				_ => 4
	...
	private bool TryLayoutColdTerminalBranch()
	{
		...
		for (var conditionalIndex = 0; conditionalIndex < instructions.Count; conditionalIndex++)
		{
			...
			var movedEntryLabels = _buffer.Labels
				.Where(label => label.Value == failureOffset ||
					label.Value >= tailOffset && label.Value < tailEnd)
				.ToArray();
			var movedEndAnchors = _buffer.AnalysisAnchors
				.Where(anchor => anchor.Value > unconditional.Offset &&
					anchor.Value <= tailEnd)
				.ToArray();
			var tailEffects = _buffer.InstructionEffectOverrides
				.Where(effect => effect.Key >= tailOffset && effect.Key < tailEnd)
				.ToArray();

			// Replace only the branch. Buffer mutations keep every section boundary
			// and retained instruction/fixup offset in sync, including the suffix.
			_buffer.RemoveBytes(unconditional.Offset, unconditional.Length);
			_buffer.InsertBytes(unconditional.Offset, tailLength);
			for (var index = 0; index < tailLength; index++)
			{
				_buffer.Bytes[unconditional.Offset + index] = tail[index];
			}

			foreach (var (label, offset) in movedEntryLabels)
			{
				_buffer.Labels[label] = offset == failureOffset
					? newFailureOffset
					: unconditional.Offset + offset - tailOffset;
			}
			foreach (var (anchor, offset) in movedEndAnchors)
			{
				// Analysis anchors mark block ends: a shared boundary follows the
				// preceding block. The failure end stays after the failure bytes,
				// while tail ends follow the first copy, like the tail entry labels.
				_buffer.AnalysisAnchors[anchor] = offset <= failureOffset
					? newFailureOffset
					: offset <= tailOffset
						? newFailureOffset + offset - failureOffset
						: unconditional.Offset + offset - tailOffset;
			}
			foreach (var (offset, effects) in tailEffects)
			{
				// The original tail retains its effects after the failure block;
				// the newly inserted success-path copy needs the same overrides.
				_buffer.InstructionEffectOverrides.Add(
					unconditional.Offset + offset - tailOffset, effects);
			}
			return true;
		}
	...
	private bool TryRemoveExactSameRegisterFrameReload()
	{
		var instructions = _assembler.GetInstructionStream();
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			if (store.Length != 4 ||
				reload.Length != 4 ||
				(store.Opcode & 0xFFF0) != 0x2F40 ||
				store.ExtensionWord != reload.ExtensionWord ||
				IsReferencedLabelAt(reload.Offset))
			{
				continue;
			}

			var sourceIsAddress = (store.Opcode & 8) != 0;
			var sourceRegister = store.Opcode & 7;
			if (sourceIsAddress && sourceRegister == 7)
			{
				// Reloading A7 changes the stack pointer and is not redundant.
				continue;
			}

			var expectedReload = sourceIsAddress
				? 0x206F | (sourceRegister << 9) // MOVEA.L d16(A7),An
				: 0x202F | (sourceRegister << 9); // MOVE.L d16(A7),Dn
			if (reload.Opcode != expectedReload)
			{
				continue;
			}

			// The store and reload observe the same compiler-owned frame slot with
			// no intervening instruction.  The register already contains the stored
			// value.  MOVEA does not affect flags; for a data register, both MOVE
			// instructions produce the same N/Z/V/C state (and leave X unchanged).
			MoveLabelsToOffset(
				reload.Offset,
				reload.Offset + reload.Length,
				store.Offset);
		...
		M68kInstructionDataflow dataflow)
	{
		...
		for (var index = 0; index + 2 < instructions.Count; index++)
		{
			...
			_buffer.InsertBytes(reload.Offset, 4, shiftBoundaryLabels: true, shiftBoundaryAnchors: true);
	...
	private bool TryFoldLeaToAddressRegisterMove(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			if ((lea.Opcode & 0xF1C0) != 0x41C0 ||
				(moveAddress.Opcode & 0xF1F8) != 0x2048 ||
				moveAddress.Length != 2 ||
				HasLabelAt(moveAddress.Offset) ||
				!dataflow.TryGetFacts(moveAddress.Offset, out var moveFacts))
			{
				continue;
			}
			...
			if ((moveAddress.Opcode & 7) != temporary ||
				(moveFacts.LiveAddressAfter & (1 << temporary)) != 0)
			{
				continue;
			}
			...
			_buffer.WriteWord(
				lea.Offset,
				(ushort)((lea.Opcode & 0xF1FF) | (destination << 9)));
	...
	private bool TryFoldDataRegisterExchange(M68kInstructionDataflow dataflow)
	{
		...
		for (var index = 0; index + 2 < instructions.Count; index++)
		{
			...
			if (facts.LiveConditionsAfter != M68kConditionCodeSet.None)
			{
				// EXG does not affect CCR, whereas the final MOVE.L did. TST.L on the
				// final destination recreates its exact N/Z/V/C state and preserves X.
				_buffer.WriteWord(second.Offset, (ushort)(0x4A80 | secondSource));
				_buffer.RemoveBytes(third.Offset, third.Length);
			}
			else
			{
				_buffer.RemoveBytes(second.Offset, second.Length);
	...
	private bool TryForwardAddressRegisterBase(M68kInstructionDataflow dataflow)
	{
		if (TryForwardLeaToMemoryLoad(dataflow) ||
			TryForwardLeaToMemoryStore(dataflow))
		{
			return true;
		}
	...
	private bool TryFoldBaseIndexMemoryAccess(M68kInstructionDataflow dataflow)
	{
		...
		for (var copyIndex = 0; copyIndex + 2 < instructions.Count; copyIndex++)
		{
				...
				addIndex++)
			{
				...
				// Grow the access instruction before the following instruction. Its
				// entry labels and the access's end anchors must skip the new extension.
				_buffer.InsertBytes(extensionOffset, 2,
					shiftBoundaryLabels: true, shiftBoundaryAnchors: true);
	...
	private bool TryForwardLeaToMemoryLoad(M68kInstructionDataflow dataflow)
	{
		...
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			if ((lea.Opcode & 0xF1C0) != 0x41C0 ||
				leaMode is not (2 or 5) ||
				lea.Length != (leaMode == 2 ? 2 : 4) ||
				loadMode is not (2 or 5) ||
				load.Length < (loadMode == 2 ? 2 : 4) ||
			...
			var destinationMode = load.Opcode >> 6 & 7;
			var destinationRegister = load.Opcode >> 9 & 7;
			if (addressRegister == 7 ||
				(load.Opcode & 7) != addressRegister ||
				destinationMode != 0 &&
				(destinationMode != 1 || (load.Opcode >> 12) != 2) &&
				!IsAlterableMemoryDestination(destinationMode, destinationRegister) ||
				DestinationUsesAddressRegister(load, destinationMode,
					destinationRegister, addressRegister) ||
			...
			var displacement = (leaMode == 5 ? (short)lea.ExtensionWord : 0) +
				(loadMode == 5 ? (short)load.ExtensionWord : 0);
			if (displacement is < short.MinValue or > short.MaxValue)
			{
				continue;
			}
			var replacementMode = displacement == 0 ? 2 : 5;
			var replacement = (ushort)((load.Opcode & ~0x3F) |
				(replacementMode << 3) | baseRegister);
			var originalSourceExtensionBytes = loadMode == 5 ? 2 : 0;
			var destinationExtensionBytes = load.Length - 2 -
				originalSourceExtensionBytes;
			var destinationExtensions = new ushort[destinationExtensionBytes / 2];
			for (var extension = 0; extension < destinationExtensions.Length;
				extension++)
			{
				destinationExtensions[extension] = _buffer.ReadWord(load.Offset + 2 +
					originalSourceExtensionBytes + extension * 2);
			}
			_buffer.WriteWord(lea.Offset, replacement);
			var replacementLength = 2;
			if (replacementMode == 5)
			{
				_buffer.WriteWord(lea.Offset + 2, unchecked((ushort)displacement));
				replacementLength = 4;
			}
			for (var extension = 0; extension < destinationExtensions.Length;
				extension++)
			{
				_buffer.WriteWord(lea.Offset + replacementLength,
					destinationExtensions[extension]);
				replacementLength += 2;
			}
			var originalEnd = load.Offset + load.Length;
			_buffer.RemoveBytes(lea.Offset + replacementLength,
				originalEnd - lea.Offset - replacementLength);
			return true;
		}
	...
	private bool TryForwardLeaToMemoryStore(M68kInstructionDataflow dataflow)
	{
		var instructions = _assembler.GetInstructionStream();
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			var temporary = lea.Opcode >> 9 & 7;
			var baseRegister = lea.Opcode & 7;
			if (temporary == 7 || temporary == baseRegister)
			{
				continue;
			}
			var temporaryMask = (ushort)(1 << temporary);
			var baseMask = (ushort)(1 << baseRegister);
			var lastStoreIndex = Math.Min(index + 3, instructions.Count - 1);
			for (var storeIndex = index + 1; storeIndex <= lastStoreIndex;
				storeIndex++)
			{
				var store = instructions[storeIndex];
				var storeMode = store.Opcode >> 6 & 7;
				var sourceMode = store.Opcode >> 3 & 7;
				if (((store.Opcode >> 12) & 0xF) is not (1 or 2 or 3) ||
					storeMode is not (2 or 5) ||
					sourceMode is not (0 or 1) ||
					(store.Opcode >> 9 & 7) != temporary ||
					(sourceMode == 1 && (store.Opcode & 7) == temporary) ||
					IsReferencedLabelAt(store.Offset) ||
					!dataflow.TryGetFacts(store.Offset, out _) ||
					!InterveningInstructionsPreserveLeaBase(
						instructions,
						index + 1,
						storeIndex,
						temporaryMask,
				...
				var displacement =
					(leaMode == 5 ? (short)lea.ExtensionWord : 0) +
					(storeMode == 5 ? (short)store.ExtensionWord : 0);
				if (displacement is < short.MinValue or > short.MaxValue ||
					(storeMode == 2 && displacement != 0) ||
					IsAddressRegisterReadBeforeOverwrite(
						instructions,
						storeIndex + 1,
						temporary,
				...
				var replacement = (ushort)((store.Opcode & ~(7 << 9)) |
					(baseRegister << 9));
				_buffer.WriteWord(store.Offset, replacement);
				if (storeMode == 5)
				{
					_buffer.WriteWord(store.Offset + 2,
						unchecked((ushort)displacement));
				}
				MoveLabelsToOffset(lea.Offset, instructions[index + 1].Offset,
					lea.Offset);
				_buffer.RemoveBytes(lea.Offset, lea.Length);
				return true;
			}
		}
	...
	private bool InterveningInstructionsPreserveLeaBase(
		IReadOnlyList<M68kEmittedInstruction> instructions,
		int startIndex,
		int endIndex,
		ushort temporaryMask,
		ushort baseMask,
		M68kInstructionDataflow dataflow)
	{
		for (var index = startIndex; index < endIndex; index++)
		...
		M68kInstructionDataflow dataflow)
	{
		...
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			if ((moveAddress.Opcode & 7) != temporary ||
				(moveFacts.LiveConditionsAfter & moveFacts.Effects.WritesConditions) !=
					M68kConditionCodeSet.None ||
				...
				(addressFacts.LiveDataAfter & (1 << temporary)) != 0 &&
					IsDataRegisterReadBeforeOverwriteWithKnownCalls(
						instructions, index + 2, temporary, dataflow,
						ref wholeImageInstructions, callTargetProofs,
						moveFacts.Effects.WritesConditions))
			{
				continue;
			}
	...
	private bool TryForwardAddressRegisterMoveIntoCompare(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			if ((move.Opcode & 0xF1F8) != 0x2008 || // MOVE.L An,Dtemporary
				move.Length != 2 ||
				(compare.Opcode & 0xF1C0) != 0xB080 || // CMP.L Dtemporary,Ddestination
				((compare.Opcode >> 3) & 7) != 0 ||
				compare.Length != 2 ||
				HasLabelAt(move.Offset) ||
				!dataflow.TryGetFacts(compare.Offset, out var compareFacts))
			{
				continue;
			}

			var addressRegister = move.Opcode & 7;
			var temporary = (move.Opcode >> 9) & 7;
			if ((compare.Opcode & 7) != temporary ||
				((compare.Opcode >> 9) & 7) == temporary ||
				(compareFacts.LiveDataAfter & (1 << temporary)) != 0)
			{
				continue;
			}

			// CMP.L accepts an address register as its source effective address. This
			// preserves the original subtraction and condition codes while removing a
			// dead data-register transport. Do not fold when the temporary is also the
			// comparison destination: that destination was defined by the MOVE.
			_buffer.WriteWord(
				compare.Offset,
				(ushort)((compare.Opcode & 0xFFC0) | 0x0008 | addressRegister));
	...
	private bool TryForwardMemoryLoadToAddressPair(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 2 < instructions.Count; index++)
		{
			var load = instructions[index];
			var firstCopy = instructions[index + 1];
			var secondCopy = instructions[index + 2];
			var sourceMode = (load.Opcode >> 3) & 7;
			var sourceRegister = load.Opcode & 7;
			if ((load.Opcode & 0xF000) != 0x2000 || // MOVE.L <ea>,Dtemp
				((load.Opcode >> 6) & 7) != 0 ||
				!IsReadableMemoryEffectiveAddress(sourceMode, sourceRegister) ||
				(firstCopy.Opcode & 0xF1F8) != 0x2040 || // MOVEA.L Dtemp,Afirst
				firstCopy.Length != 2 ||
				(secondCopy.Opcode & 0xF1F8) != 0x2040 || // MOVEA.L Dtemp,Asecond
				secondCopy.Length != 2 ||
				HasLabelAt(firstCopy.Offset) ||
				HasLabelAt(secondCopy.Offset) ||
				!dataflow.TryGetFacts(load.Offset, out var loadFacts) ||
				!dataflow.TryGetFacts(secondCopy.Offset, out var secondFacts))
			{
				continue;
			}

			var temporary = (load.Opcode >> 9) & 7;
			var firstAddress = (firstCopy.Opcode >> 9) & 7;
			if ((firstCopy.Opcode & 7) != temporary ||
				(secondCopy.Opcode & 7) != temporary ||
				loadFacts.LiveConditionsAfter != M68kConditionCodeSet.None ||
				(secondFacts.LiveDataAfter & (1 << temporary)) != 0)
			{
				continue;
			}

			// The data temporary is copied only into address registers. Retarget the
			// memory read to the first address register, then copy that address to the
			// second. MOVEA has no condition-code effect, so this is valid only when
			// the original MOVE.L flags are dead.
			_buffer.WriteWord(
				load.Offset,
				(ushort)((load.Opcode & 0xF03F) | (firstAddress << 9) | 0x0040));
			_buffer.WriteWord(
				secondCopy.Offset,
				(ushort)((secondCopy.Opcode & 0xFFF8) | 0x0008 | firstAddress));
			_buffer.RemoveBytes(firstCopy.Offset, firstCopy.Length);
			return true;
		}
	...
	private bool TryForwardRegisterCopyToMemory(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			var destinationMode = (store.Opcode >> 6) & 7;
			var destinationRegister = (store.Opcode >> 9) & 7;
			var sourceMode = (copy.Opcode >> 3) & 7;
			if ((copy.Opcode & 0xF000) != 0x2000 || // MOVE.L Dn/An,Dtemp
				((copy.Opcode >> 6) & 7) != 0 ||
				copy.Length != 2 ||
				sourceMode is not (0 or 1) ||
			...
			if (sourceMode == 1 && storeSize == 1)
			{
				// Address-register direct has no byte-sized MOVE encoding.
				continue;
			}

			var source = copy.Opcode & 7;
			var temporary = copy.Opcode >> 9 & 7;
			if ((sourceMode == 0 && source == temporary) ||
				(store.Opcode & 7) != temporary ||
			...
			// value and publishes the final flags. Read the same bits directly from
			// the source data or address register when the temporary dies here.
			_buffer.WriteWord(
				store.Offset,
				(ushort)((store.Opcode & ~0x003F) | (copy.Opcode & 0x003F)));
			_buffer.RemoveBytes(copy.Offset, copy.Length);
			return true;
		}
	...
	private bool IsDataRegisterReadBeforeOverwriteWithKnownCalls(
		...
		M68kConditionCodeSet changedConditions = M68kConditionCodeSet.None)
	{
		...
		var indexByOffset = instructions
			.Select((instruction, index) => (instruction.Offset, index))
			.ToDictionary(static item => item.Offset, static item => item.index);
		var pending = new Stack<(int Index, M68kConditionCodeSet Conditions)>();
		var visited = new HashSet<(int Index, M68kConditionCodeSet Conditions)>();
		var registerMask = (ushort)(1 << register);
		pending.Push((startIndex, changedConditions));
		...
		while (pending.TryPop(out var state))
		{
			...
			var (index, conditions) = state;
			var instruction = instructions[index];
			if (instruction.Kind == M68kInstructionKind.Return || HasControlTransferAddend(instruction))
			{
				return true;
			}
			if (instruction.Kind == M68kInstructionKind.Call)
			{
				if (instruction.ExternalTarget ||
					...
					CallTargetReadsDataRegisterBeforeOverwrite(
						...
						ref wholeImageInstructions,
						callTargetProofs) ||
					conditions != M68kConditionCodeSet.None &&
						CallTargetReadsConditionsBeforeOverwrite(
							targetOffset, conditions, wholeImageInstructions!))
				{
					return true;
				}
			...
			if ((facts.Effects.UsesData & registerMask) != 0 ||
				(facts.Effects.ReadsConditions & conditions) != M68kConditionCodeSet.None)
			{
				return true;
			}
			if (IsKnownFullDataRegisterOverwrite(
			...
			conditions &= ~facts.Effects.WritesConditions;
			...
			bool PushTarget(int? targetOffset)
			{
				...
				pending.Push((targetIndex, conditions));
				return true;
			}
			...
			bool PushNext()
			{
				...
				pending.Push((nextIndex, conditions));
				return true;
			}
		}
		...
		Dictionary<(int TargetOffset, int Register), bool> proofs)
	{
		...
		while (pending.TryPop(out var index))
		{
			...
			if (instruction.Offset >= methodEnd ||
				HasControlTransferAddend(instruction) ||
	...
	private bool TryGroupContiguousLongMemoryCopy(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var start = 0; start + 2 < instructions.Count; start++)
		{
			if (!TryDecodeCopy(instructions[start], out var sourceBase,
				out var destinationBase, out var firstDisplacement) ||
				sourceBase == destinationBase ||
				HasAddressFixup(instructions[start]))
			{
				continue;
			}

			var run = 1;
			while (start + run < instructions.Count && run < 8 &&
				TryDecodeCopy(instructions[start + run], out var nextSource,
					out var nextDestination, out var nextDisplacement) &&
				nextSource == sourceBase && nextDestination == destinationBase &&
				nextDisplacement == firstDisplacement + run * 4 &&
				!HasLabelAt(instructions[start + run].Offset) &&
				!HasAddressFixup(instructions[start + run]))
			{
				run++;
			}
			if (run < 3 || !dataflow.TryGetFacts(
				instructions[start + run - 1].Offset, out var facts) ||
				facts.LiveConditionsAfter != M68kConditionCodeSet.None)
			{
				continue;
			}

			var available = (ushort)(
				(~facts.LiveDataAfter & 0x00FF) |
				((~facts.LiveAddressAfter & 0x00FF) << 8));
			available &= (ushort)~(1 << (8 + sourceBase));
			available &= (ushort)~(1 << (8 + destinationBase));
			ushort mask = 0;
			var count = 0;
			for (var register = 0; register < 16 && count < run; register++)
			{
				if ((available & (1 << register)) == 0) continue;
				mask |= (ushort)(1 << register);
				count++;
			}
			if (count < 3)
			{
			...
			for (var index = 0; index < count; index++)
				originalLength += instructions[start + index].Length;
			var replacementLength = (firstDisplacement == 0 ? 4 : 6) * 2;
			if (replacementLength >= originalLength)
			{
				continue;
			}

			var offset = instructions[start].Offset;
			offset += WriteMovem(offset, load: true, sourceBase,
				firstDisplacement, mask);
			offset += WriteMovem(offset, load: false, destinationBase,
				firstDisplacement, mask);
			_buffer.RemoveBytes(offset, originalLength - replacementLength);
			return true;
		}
		return false;

		bool TryDecodeCopy(M68kEmittedInstruction instruction,
			out int sourceBase, out int destinationBase, out int displacement)
		{
			sourceBase = destinationBase = displacement = 0;
			var opcode = instruction.Opcode;
			if ((opcode & 0xF000) != 0x2000) return false;
			var sourceMode = opcode >> 3 & 7;
			var destinationMode = opcode >> 6 & 7;
			if (sourceMode is not (2 or 5) || destinationMode is not (2 or 5))
				return false;
			sourceBase = opcode & 7;
			destinationBase = opcode >> 9 & 7;
			var sourceDisplacement = sourceMode == 5
				? unchecked((short)instruction.ExtensionWord) : 0;
			var destinationExtensionOffset = instruction.Offset + 2 +
				(sourceMode == 5 ? 2 : 0);
			var destinationDisplacement = destinationMode == 5
				? unchecked((short)_buffer.ReadWord(destinationExtensionOffset)) : 0;
			if (sourceDisplacement != destinationDisplacement ||
				instruction.Length != 2 + (sourceMode == 5 ? 2 : 0) +
					(destinationMode == 5 ? 2 : 0)) return false;
			displacement = sourceDisplacement;
			return true;
		}

		int WriteMovem(int offset, bool load, int addressRegister,
			int displacement, ushort mask)
		{
			var mode = displacement == 0 ? 2 : 5;
			_buffer.WriteWord(offset, (ushort)((load ? 0x4CC0 : 0x48C0) |
				(mode << 3) | addressRegister));
			_buffer.WriteWord(offset + 2, mask);
			if (displacement == 0) return 4;
			_buffer.WriteWord(offset + 4, unchecked((ushort)displacement));
			return 6;
		}
	}

	private bool TryGroupContiguousLongMemoryPush(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var start = 0; start + 2 < instructions.Count; start++)
		{
			if (!TryDecodePush(instructions[start], out var sourceBase,
				out var firstDisplacement) || sourceBase == 7 ||
				HasAddressFixup(instructions[start]))
			{
				continue;
			}
			...
			var run = 1;
			while (start + run < instructions.Count && run < 16 &&
				TryDecodePush(instructions[start + run], out var nextSource,
					out var nextDisplacement) &&
				nextSource == sourceBase &&
				nextDisplacement == firstDisplacement - run * 4 &&
				!HasLabelAt(instructions[start + run].Offset) &&
				!HasAddressFixup(instructions[start + run]))
			{
			...
			for (var count = run; count >= 3; count--)
			{
				var last = instructions[start + count - 1];
				if (!dataflow.TryGetFacts(last.Offset, out var facts) ||
					facts.LiveConditionsAfter != M68kConditionCodeSet.None)
				{
					continue;
				...
				var available = (ushort)(
					...
					((~facts.LiveAddressAfter & 0x00FF) << 8));
				available &= (ushort)~(1 << (8 + sourceBase));
				available &= 0x7FFF; // A7 is the destination stack pointer.
				ushort mask = 0;
				var selected = 0;
				for (var register = 0; register < 16 && selected < count; register++)
				{
					if ((available & (1 << register)) == 0) continue;
					mask |= (ushort)(1 << register);
					selected++;
				...
				var lowestDisplacement = firstDisplacement - (count - 1) * 4;
				var originalLength = 0;
				for (var index = 0; index < count; index++)
					originalLength += instructions[start + index].Length;
				var loadLength = lowestDisplacement == 0 ? 4 : 6;
				const int storeLength = 4;
				if (loadLength + storeLength >= originalLength) continue;

				var offset = instructions[start].Offset;
				offset += WriteLoadMovem(offset, sourceBase,
					lowestDisplacement, mask);
				_buffer.WriteWord(offset, 0x48E7); // MOVEM.L registers,-(A7)
				_buffer.WriteWord(offset + 2, ReverseBits(mask));
				offset += storeLength;
				_buffer.RemoveBytes(offset,
					originalLength - loadLength - storeLength);
				return true;
			}
		}
		return false;

		bool TryDecodePush(M68kEmittedInstruction instruction,
			out int sourceBase, out int displacement)
		{
			sourceBase = displacement = 0;
			if ((instruction.Opcode & 0xFFC0) != 0x2F00) return false;
			var sourceMode = instruction.Opcode >> 3 & 7;
			if (sourceMode is not (2 or 5)) return false;
			sourceBase = instruction.Opcode & 7;
			displacement = sourceMode == 5
				? unchecked((short)instruction.ExtensionWord) : 0;
			return instruction.Length == (sourceMode == 5 ? 4 : 2);
		}

		static ushort ReverseBits(ushort value)
		{
			ushort result = 0;
			for (var bit = 0; bit < 16; bit++)
			{
				if ((value & (1 << bit)) != 0)
					result |= (ushort)(1 << (15 - bit));
			}
			return result;
		}

		int WriteLoadMovem(int offset, int addressRegister,
			int displacement, ushort mask)
		{
			var mode = displacement == 0 ? 2 : 5;
			_buffer.WriteWord(offset,
				(ushort)(0x4CC0 | (mode << 3) | addressRegister));
			_buffer.WriteWord(offset + 2, mask);
			if (displacement == 0) return 4;
			_buffer.WriteWord(offset + 4, unchecked((ushort)displacement));
			return 6;
	...
	private bool TryFoldLeafMemoryReadCall()
	{
		var instructions = _assembler.GetInstructionStream();
		for (var index = 0; index + 3 < instructions.Count; index++)
		{
			...
			var call = instructions[index + 2];
			var cleanup = instructions[index + 3];
			if (!TryDecodeDirectLongRegisterPush(push, out var offsetRegister) ||
				!TryDecodeDirectLongRegisterMove(receiver, destination: 0, out _) ||
				offsetRegister == 1 || offsetRegister == 8 ||
				call.Kind != M68kInstructionKind.Call ||
				call.Length is not (4 or 6) ||
				cleanup.Opcode != 0x588F || cleanup.Length != 2 ||
				HasLabelAt(receiver.Offset) && !CanRelocateLabels(receiver.Offset) ||
				HasLabelAt(call.Offset) ||
				HasLabelAt(cleanup.Offset) && !CanRelocateLabels(cleanup.Offset) ||
				!TryResolveInternalLeafCall(call, out var targetOffset,
					out var branchIndex, out var addressIndex) ||
				!TryGetLeafMemoryReadWidth(targetOffset, out var width))
			{
				continue;
			}

			var originalLength = cleanup.Offset + cleanup.Length - push.Offset;
			var replacementLength = width == 4 ? 6 : 8;
			if (originalLength <= replacementLength) continue;

			if (branchIndex >= 0) _buffer.Branches.RemoveAt(branchIndex);
			if (addressIndex >= 0) _buffer.Addresses.RemoveAt(addressIndex);
			MoveLabelsToOffset(receiver.Offset,
				receiver.Offset + receiver.Length, push.Offset);
			MoveLabelsToOffset(cleanup.Offset,
				cleanup.Offset + cleanup.Length, push.Offset + replacementLength);
			_buffer.WriteWord(push.Offset, 0x2041); // MOVEA.L D1,A0
			_buffer.WriteWord(push.Offset + 2, (ushort)(
				offsetRegister < 8
					? 0xD1C0 | offsetRegister // ADDA.L Dn,A0
					: 0xD1C8 | (offsetRegister - 8))); // ADDA.L An,A0
			if (width == 4)
				_buffer.WriteWord(push.Offset + 4, 0x2010); // MOVE.L (A0),D0
			else
			{
				_buffer.WriteWord(push.Offset + 4, 0x7000); // MOVEQ #0,D0
				_buffer.WriteWord(push.Offset + 6,
					width == 1 ? 0x1010 : 0x3010); // MOVE.B/W (A0),D0
			}
			_buffer.RemoveBytes(push.Offset + replacementLength,
	...
	private bool TryRemoveCallToRtsLeaf()
	{
		var instructions = _assembler.GetInstructionStream();
		foreach (var call in instructions)
		{
			if (call.Kind != M68kInstructionKind.Call ||
				call.IsNonReturning ||
				call.Length is not (4 or 6) ||
				(HasLabelAt(call.Offset) && !CanRelocateLabels(call.Offset)) ||
				!TryResolveInternalLeafCall(call, out var targetOffset,
					out var branchIndex, out var addressIndex) ||
				targetOffset < 0 || targetOffset + 1 >= _buffer.Bytes.Count ||
				_buffer.ReadWord(targetOffset) != 0x4E75)
			{
				continue;
			}
			...
			if (branchIndex >= 0) _buffer.Branches.RemoveAt(branchIndex);
			if (addressIndex >= 0) _buffer.Addresses.RemoveAt(addressIndex);
			MoveLabelsToOffset(call.Offset, call.Offset + call.Length, call.Offset);
	...
	private bool TryFoldLeafMemoryReadCallWithLateAddress()
	{
		var instructions = _assembler.GetInstructionStream();
		for (var index = 0; index + 4 < instructions.Count; index++)
		{
			var push = instructions[index];
			var receiver = instructions[index + 1];
			var address = instructions[index + 2];
			var call = instructions[index + 3];
			var cleanup = instructions[index + 4];
			if (!TryDecodeDirectLongRegisterPush(push, out var offsetRegister) ||
				!TryDecodeDirectLongRegisterMove(receiver, destination: 0, out _) ||
				!TryDecodeDirectLongRegisterMove(address, destination: 1, out _) ||
				offsetRegister == 1 || offsetRegister == 8 ||
				call.Kind != M68kInstructionKind.Call ||
				call.Length is not (4 or 6) ||
				cleanup.Opcode != 0x588F || cleanup.Length != 2 ||
				HasLabelAt(receiver.Offset) && !CanRelocateLabels(receiver.Offset) ||
				HasLabelAt(address.Offset) && !CanRelocateLabels(address.Offset) ||
				HasLabelAt(call.Offset) ||
				HasLabelAt(cleanup.Offset) && !CanRelocateLabels(cleanup.Offset) ||
				!TryResolveInternalLeafCall(call, out var targetOffset,
					out var branchIndex, out var addressIndex) ||
			...
			var originalLength = cleanup.Offset + cleanup.Length - push.Offset;
			var replacementLength = width == 4 ? 8 : 10;
			if (originalLength <= replacementLength) continue;
			if (branchIndex >= 0) _buffer.Branches.RemoveAt(branchIndex);
			if (addressIndex >= 0) _buffer.Addresses.RemoveAt(addressIndex);
			MoveLabelsToOffset(receiver.Offset,
				address.Offset + address.Length, push.Offset);
			MoveLabelsToOffset(cleanup.Offset,
				cleanup.Offset + cleanup.Length, push.Offset + replacementLength);
			_buffer.WriteWord(push.Offset, address.Opcode); // MOVE.L address,D1
			_buffer.WriteWord(push.Offset + 2, 0x2041); // MOVEA.L D1,A0
			_buffer.WriteWord(push.Offset + 4, (ushort)(
				offsetRegister < 8
					? 0xD1C0 | offsetRegister
					: 0xD1C8 | (offsetRegister - 8)));
			if (width == 4)
				_buffer.WriteWord(push.Offset + 6, 0x2010); // MOVE.L (A0),D0
			else
			{
				_buffer.WriteWord(push.Offset + 6, 0x7000); // MOVEQ #0,D0
				_buffer.WriteWord(push.Offset + 8,
					width == 1 ? 0x1010 : 0x3010); // MOVE.B/W (A0),D0
			}
			_buffer.RemoveBytes(push.Offset + replacementLength,
				originalLength - replacementLength);
	...
	private static bool TryDecodeDirectLongRegisterPush(
		M68kEmittedInstruction instruction, out int register)
	{
		register = 0;
		if (instruction.Length != 2) return false;
		if ((instruction.Opcode & 0xFFF8) == 0x2F00)
		{
			register = instruction.Opcode & 7;
			return true;
		}
		if ((instruction.Opcode & 0xFFF8) != 0x2F08) return false;
		register = 8 + (instruction.Opcode & 7);
		return true;
	}

	private static bool TryDecodeDirectLongRegisterMove(
		M68kEmittedInstruction instruction, int destination, out int source)
	{
		source = 0;
		if (instruction.Length != 2 ||
			(instruction.Opcode & 0xF000) != 0x2000 ||
			(instruction.Opcode >> 6 & 7) != 0 ||
			(instruction.Opcode >> 9 & 7) != destination)
		{
			return false;
		}
		var mode = instruction.Opcode >> 3 & 7;
		if (mode is not (0 or 1)) return false;
		source = (mode == 0 ? 0 : 8) + (instruction.Opcode & 7);
		return true;
	}

	private bool TryResolveInternalLeafCall(M68kEmittedInstruction call,
		out int targetOffset, out int branchIndex, out int addressIndex)
	{
		targetOffset = 0;
		branchIndex = addressIndex = -1;
		for (var candidate = 0; candidate < _buffer.Branches.Count; candidate++)
		{
			var fixup = _buffer.Branches[candidate];
			if (fixup.OpcodeOffset != call.Offset ||
				!_buffer.Labels.TryGetValue(fixup.Target, out targetOffset)) continue;
			branchIndex = candidate;
			return true;
		}
		for (var candidate = 0; candidate < _buffer.Addresses.Count; candidate++)
		{
			var fixup = _buffer.Addresses[candidate];
			if (fixup.Offset != call.Offset + 2 || fixup.External ||
				!_buffer.Labels.TryGetValue(fixup.Target, out targetOffset)) continue;
	...
	private bool TryGetLeafMemoryReadWidth(int offset, out int width)
	{
		width = 0;
		if (offset < 0 || offset + 11 >= _buffer.Bytes.Count ||
			_buffer.ReadWord(offset) != 0x202F ||
			_buffer.ReadWord(offset + 2) != 4 ||
			_buffer.ReadWord(offset + 4) != 0x2041 ||
			_buffer.ReadWord(offset + 6) != 0xD1C0)
		{
			return false;
		}
		if (_buffer.ReadWord(offset + 8) == 0x2010 &&
			_buffer.ReadWord(offset + 10) == 0x4E75)
		{
			width = 4;
			return true;
		}
		if (offset + 13 >= _buffer.Bytes.Count ||
			_buffer.ReadWord(offset + 8) != 0x7000 ||
			_buffer.ReadWord(offset + 12) != 0x4E75)
		{
			return false;
		}
		var load = _buffer.ReadWord(offset + 10);
		if (load == 0x1010) width = 1;
		else if (load == 0x3010) width = 2;
		return width != 0;
	}

	private bool TryGetLeafMemoryWriteWidth(int offset, out int width)
	{
		width = 0;
		if (offset < 0 || offset + 21 >= _buffer.Bytes.Count ||
			_buffer.ReadWord(offset) != 0x2F02 ||
			_buffer.ReadWord(offset + 2) != 0x242F ||
			_buffer.ReadWord(offset + 4) != 12 ||
			_buffer.ReadWord(offset + 6) != 0x202F ||
			_buffer.ReadWord(offset + 8) != 8 ||
			_buffer.ReadWord(offset + 10) != 0x2041)
		{
			return false;
		}
		if (_buffer.ReadWord(offset + 12) == 0x2202 &&
			_buffer.ReadWord(offset + 14) == 0xD1C0 &&
			_buffer.ReadWord(offset + 16) == 0x2081 &&
			_buffer.ReadWord(offset + 18) == 0x241F &&
			_buffer.ReadWord(offset + 20) == 0x4E75)
		{
			width = 4;
			return true;
		}
		if (offset + 23 >= _buffer.Bytes.Count ||
			_buffer.ReadWord(offset + 12) != 0x7200 ||
			_buffer.ReadWord(offset + 16) != 0xD1C0 ||
			_buffer.ReadWord(offset + 20) != 0x241F ||
			_buffer.ReadWord(offset + 22) != 0x4E75)
		{
			return false;
		}
		var narrow = _buffer.ReadWord(offset + 14);
		var store = _buffer.ReadWord(offset + 18);
		if (narrow == 0x1202 && store == 0x1081) width = 1;
		else if (narrow == 0x3202 && store == 0x3081) width = 2;
		return width != 0;
	}

	private bool TryFoldConstantAddressRead(M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 4 < instructions.Count; index++)
		{
			var constant = instructions[index];
			var address = instructions[index + 1];
			var moveAddress = instructions[index + 2];
			var addOffset = instructions[index + 3];
			var load = instructions[index + 4];
			if ((constant.Opcode & 0xF100) != 0x7000 ||
				constant.Length != 2 ||
				!TryGetDirectRegisterMove(address, destination: 1,
					out var addressSource) ||
				moveAddress.Opcode != 0x2041 || moveAddress.Length != 2 ||
				addOffset.Opcode != (0xD1C0 | (constant.Opcode >> 9 & 7)) ||
				addOffset.Length != 2 ||
				load.Opcode != 0x2010 || load.Length != 2 ||
				HasLabelAt(address.Offset) && !CanRelocateLabels(address.Offset) ||
				HasLabelAt(moveAddress.Offset) && !CanRelocateLabels(moveAddress.Offset) ||
				HasLabelAt(addOffset.Offset) && !CanRelocateLabels(addOffset.Offset) ||
				HasLabelAt(load.Offset) && !CanRelocateLabels(load.Offset) ||
				!dataflow.TryGetFacts(load.Offset, out var facts))
			{
				continue;
			}
			...
			if (constantRegister != 0 &&
				(facts.LiveDataAfter & (1 << constantRegister)) != 0)
			{
				continue;
			}
			var displacement = unchecked((sbyte)constant.Opcode);
			var keepD1 = (facts.LiveDataAfter & (1 << 1)) != 0;
			var offset = constant.Offset;
			if (keepD1)
			{
				_buffer.WriteWord(offset, address.Opcode);
				_buffer.WriteWord(offset + 2, 0x2041); // MOVEA.L D1,A0
				offset += 4;
			}
			else
			{
				_buffer.WriteWord(offset, (ushort)(addressSource < 8
					? 0x2040 | addressSource
					: 0x2048 | (addressSource - 8))); // MOVEA.L source,A0
				offset += 2;
			}
			if (displacement == 0)
			{
				_buffer.WriteWord(offset, 0x2010); // MOVE.L (A0),D0
				offset += 2;
			}
			else
			{
				_buffer.WriteWord(offset, 0x2028); // MOVE.L d16(A0),D0
				_buffer.WriteWord(offset + 2, unchecked((ushort)displacement));
				offset += 4;
			}
			var originalEnd = load.Offset + load.Length;
			MoveLabelsToOffset(address.Offset, originalEnd, constant.Offset);
			_buffer.RemoveBytes(offset, originalEnd - offset);
			return true;
		}
		return false;

		static bool TryGetDirectRegisterMove(M68kEmittedInstruction instruction,
			int destination, out int source)
		{
			source = 0;
			if (instruction.Length != 2 ||
				(instruction.Opcode & 0xF000) != 0x2000 ||
	...
	private bool TryFoldLeafMemoryWriteCall(M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 4 < instructions.Count; index++)
		{
			var valuePush = instructions[index];
			var offsetPush = instructions[index + 1];
			var receiver = instructions[index + 2];
			var call = instructions[index + 3];
			var cleanup = instructions[index + 4];
			if (!TryGetMemoryPushSource(valuePush, out var valueMode,
					out var valueRegister) ||
				!TryGetDirectRegisterPush(offsetPush, out var offsetRegister) ||
				offsetRegister == 8 ||
				valueMode is 1 or 2 or 5 && valueRegister == 0 ||
				!TryGetDirectRegisterMove(receiver, destination: 0, out _) ||
				call.Kind != M68kInstructionKind.Call || call.Length is not (4 or 6) ||
				cleanup.Opcode != 0x508F || cleanup.Length != 2 ||
				HasLabelAt(offsetPush.Offset) && !CanRelocateLabels(offsetPush.Offset) ||
				HasLabelAt(receiver.Offset) && !CanRelocateLabels(receiver.Offset) ||
				HasLabelAt(call.Offset) ||
				HasLabelAt(cleanup.Offset) && !CanRelocateLabels(cleanup.Offset) ||
				!TryResolveInternalCall(call, out var targetOffset,
					out var branchIndex, out var addressIndex) ||
				!TryGetLeafMemoryWriteWidth(targetOffset, out var width) ||
				!dataflow.TryGetFacts(cleanup.Offset, out var facts) ||
				(facts.LiveDataAfter & 0x0003) != 0 ||
				facts.LiveConditionsAfter != M68kConditionCodeSet.None)
			{
				continue;
			}

			var originalLength = cleanup.Offset + cleanup.Length - valuePush.Offset;
			var replacementLength = 6 + valuePush.Length;
			if (replacementLength >= originalLength) continue;
			if (branchIndex >= 0) _buffer.Branches.RemoveAt(branchIndex);
			if (addressIndex >= 0) _buffer.Addresses.RemoveAt(addressIndex);
			MoveLabelsToOffset(offsetPush.Offset,
				call.Offset + call.Length, valuePush.Offset);
			MoveLabelsToOffset(cleanup.Offset,
				cleanup.Offset + cleanup.Length,
				valuePush.Offset + replacementLength);
			var offset = valuePush.Offset;
			_buffer.WriteWord(offset, 0x2041); // MOVEA.L D1,A0
			_buffer.WriteWord(offset + 2, (ushort)(offsetRegister < 8
				? 0xD1C0 | offsetRegister
				: 0xD1C8 | (offsetRegister - 8))); // ADDA.L offset,A0
			offset += 4;
			_buffer.WriteWord(offset,
				0x2000 | (valuePush.Opcode & 0x003F)); // MOVE.L value,D0
			if (valuePush.Length >= 4)
				_buffer.WriteWord(offset + 2, valuePush.ExtensionWord);
			if (valuePush.Length >= 6)
				_buffer.WriteWord(offset + 4, (ushort)valuePush.ExtensionLong);
			offset += valuePush.Length;
			_buffer.WriteWord(offset, width switch
			{
				1 => 0x1080, // MOVE.B D0,(A0)
				2 => 0x3080, // MOVE.W D0,(A0)
				_ => 0x2080, // MOVE.L D0,(A0)
			});
			offset += 2;
			_buffer.RemoveBytes(offset, originalLength - replacementLength);
			return true;
		...
		static bool TryGetMemoryPushSource(M68kEmittedInstruction instruction,
			out int mode, out int register)
		{
			mode = register = 0;
			if ((instruction.Opcode & 0xFFC0) != 0x2F00) return false;
			mode = instruction.Opcode >> 3 & 7;
			register = instruction.Opcode & 7;
			if (mode is not (0 or 1 or 2 or 5)) return false;
			return instruction.Length == (mode == 5 ? 4 : 2);
		}

		static bool TryGetDirectRegisterPush(
			M68kEmittedInstruction instruction, out int register)
		{
			register = 0;
			if (instruction.Length != 2) return false;
		...
		bool TryResolveInternalCall(M68kEmittedInstruction call,
			out int targetOffset, out int branchIndex, out int addressIndex)
		{
			targetOffset = 0;
			branchIndex = addressIndex = -1;
	...
	private bool TryFoldLeafMemoryWriteCallWithLateAddress(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 5 < instructions.Count; index++)
		{
			var valuePush = instructions[index];
			var offsetPush = instructions[index + 1];
			var receiver = instructions[index + 2];
			var address = instructions[index + 3];
			var call = instructions[index + 4];
			var cleanup = instructions[index + 5];
			if (!TryGetMemoryPushSource(valuePush, out var valueMode,
					out var valueRegister) ||
				!TryGetDirectRegisterPush(offsetPush, out var offsetRegister) ||
				offsetRegister == 8 ||
			...
			var valueUsesA0 = valueMode is 1 or 2 or 5 && valueRegister == 0;
			if (valueUsesA0 && (addressRegister == 0 || offsetRegister == 0))
				continue;
			var originalLength = cleanup.Offset + cleanup.Length - valuePush.Offset;
			var replacementLength = 6 + valuePush.Length;
			if (replacementLength >= originalLength) continue;
			...
			if (valueUsesA0)
			{
				EmitValueLoad(valuePush, offset);
				offset += valuePush.Length;
			}
			EmitAddressLoad(addressRegister, offset);
			offset += 2;
			_buffer.WriteWord(offset, (ushort)(offsetRegister < 8
				? 0xD1C0 | offsetRegister
				: 0xD1C8 | (offsetRegister - 8))); // ADDA.L offset,A0
			offset += 2;
			if (!valueUsesA0)
		...
		void EmitValueLoad(M68kEmittedInstruction valuePush, int offset)
		{
			_buffer.WriteWord(offset, 0x2000 | (valuePush.Opcode & 0x003F));
			if (valuePush.Length >= 4)
				_buffer.WriteWord(offset + 2, valuePush.ExtensionWord);
			if (valuePush.Length >= 6)
				_buffer.WriteWord(offset + 4, (ushort)valuePush.ExtensionLong);
		...
		void EmitAddressLoad(int sourceRegister, int offset) =>
			_buffer.WriteWord(offset, (ushort)(sourceRegister < 8
				? 0x2040 | sourceRegister
				: 0x2048 | (sourceRegister - 8))); // MOVEA.L source,A0
	...
	private bool TryFoldConstantDirectAddressRead(M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 3 < instructions.Count; index++)
		{
			var constant = instructions[index];
			var moveAddress = instructions[index + 1];
			var addOffset = instructions[index + 2];
			var load = instructions[index + 3];
			if ((constant.Opcode & 0xF100) != 0x7000 || constant.Length != 2 ||
				!IsDirectRegisterMoveA0(moveAddress) ||
				addOffset.Opcode != (0xD1C0 | (constant.Opcode >> 9 & 7)) ||
				addOffset.Length != 2 ||
				load.Opcode != 0x2010 || load.Length != 2 ||
				HasLabelAt(moveAddress.Offset) && !CanRelocateLabels(moveAddress.Offset) ||
			...
			var displacement = unchecked((sbyte)constant.Opcode);
			var offset = constant.Offset;
			_buffer.WriteWord(offset, moveAddress.Opcode);
			offset += 2;
			if (displacement == 0)
			{
				_buffer.WriteWord(offset, 0x2010);
				offset += 2;
			}
			else
			{
				_buffer.WriteWord(offset, 0x2028);
				_buffer.WriteWord(offset + 2, unchecked((ushort)displacement));
				offset += 4;
			}
			var originalEnd = load.Offset + load.Length;
			MoveLabelsToOffset(moveAddress.Offset, originalEnd, constant.Offset);
			_buffer.RemoveBytes(offset, originalEnd - offset);
			return true;
		}
		return false;

		static bool IsDirectRegisterMoveA0(M68kEmittedInstruction instruction)
		{
			if (instruction.Length != 2 ||
				(instruction.Opcode & 0xF000) != 0x2000 ||
				(instruction.Opcode >> 6 & 7) != 1 ||
				(instruction.Opcode >> 9 & 7) != 0)
				return false;
			return (instruction.Opcode >> 3 & 7) is 0 or 1;
		}
	}
	...
	private bool TryFoldAdjacentStackReleases()
	{
		var instructions = _assembler.GetInstructionStream();
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			var first = instructions[index];
			var second = instructions[index + 1];
			if (first.Opcode != 0x4FEF || first.Length != 4 ||
				second.Opcode != 0x4FEF || second.Length != 4 ||
				IsReferencedLabelAt(first.Offset) || IsReferencedLabelAt(second.Offset) ||
				HasAddressFixupAt(first.Offset + 2) || HasAddressFixupAt(second.Offset + 2))
			{
				continue;
			}
			var combined = unchecked((short)first.ExtensionWord) + unchecked((short)second.ExtensionWord);
			if (combined is <= 0 or > short.MaxValue)
			{
				continue;
			}
			_buffer.WriteWord(first.Offset + 2, combined);
			_buffer.RemoveBytes(second.Offset, second.Length);
			return true;
		}
		return false;
	...
	private bool TryRemoveUnusedRegisterStackPreservation()
	{
		var instructions = _assembler.GetInstructionStream();
		for (var index = 0; index + 2 < instructions.Count; index++)
		{
			...
			if ((push.Opcode & 0xFFF8) != 0x2F00 || push.Length != 2 ||
				(pop.Opcode & 0xF1FF) != 0x201F || pop.Length != 2 ||
				((push.Opcode & 7) != ((pop.Opcode >> 9) & 7)) ||
				move.Length != 6 || (move.Opcode & 0x3000) != 0x2000 ||
				((move.Opcode >> 3) & 7) != 5 ||
				(move.Opcode & 7) != 7 || ((move.Opcode >> 6) & 7) != 5 ||
				((move.Opcode >> 9) & 7) != 7 ||
				IsReferencedLabelAt(push.Offset) || IsReferencedLabelAt(move.Offset) ||
				IsReferencedLabelAt(pop.Offset) || HasAddressFixupAt(move.Offset + 2) ||
				HasAddressFixupAt(move.Offset + 4))
			{
				continue;
			}
			var source = unchecked((short)move.ExtensionWord) - 4;
			var destination = unchecked((short)_buffer.ReadWord(move.Offset + 4)) - 4;
			_buffer.WriteWord(move.Offset + 2, source);
			_buffer.WriteWord(move.Offset + 4, destination);
	...
	private bool TryRewriteStackPreservation(M68kInstructionDataflow dataflow)
	{
		...
		for (var index = 0; index + 2 < instructions.Count; index++)
		{
			...
			var canonicalZeroDisplacementShuffle =
				shuffle.Opcode == 0x2F57 && shuffle.Length == 4;
			if (store.Length != 2 ||
				(store.Opcode & 0xFFF8) != 0x2E80 ||
				constant.Length != 2 ||
				(constant.Opcode & 0xF100) != 0x7000 ||
				(!canonicalZeroDisplacementShuffle &&
			...
			{
				if (canonicalZeroDisplacementShuffle)
				{
					// Make room for MOVE.L scratch,d16(A7); the zero-displacement
					// source was already shortened from d16(A7) to (A7).
					_buffer.InsertBytes(shuffle.Offset + 2, 2);
				}
				_buffer.WriteWord(
					store.Offset,
			...
			var destinationDisplacement = _buffer.ReadWord(
				shuffle.Offset + (canonicalZeroDisplacementShuffle ? 2 : 4));
			_buffer.WriteWord(store.Offset, 0x2F40 | sourceRegister);
			_buffer.WriteWord(store.Offset + 2, destinationDisplacement);
		...
		M68kInstructionDataflow dataflow)
	{
		...
		{
			var copy = instructions[copyIndex];
			var adjacentNormalization = instructions[copyIndex + 1];
			if ((copy.Opcode & 0xF1F8) == 0x2000 &&
				copy.Length == 2 &&
				(copy.Opcode & 7) != ((copy.Opcode >> 9) & 7) &&
				!IsReferencedLabelAt(adjacentNormalization.Offset) &&
				(adjacentNormalization.Opcode & 0xFFF8) == 0x0280 &&
				adjacentNormalization.Length == 6 &&
				adjacentNormalization.ExtensionLong == 0x0000FFFF &&
				(adjacentNormalization.Opcode & 7) == ((copy.Opcode >> 9) & 7) &&
				dataflow.TryGetFacts(adjacentNormalization.Offset, out var adjacentFacts) &&
				(adjacentFacts.LiveConditionsAfter & logicalConditions) == 0)
			{
				var longSource = copy.Opcode & 7;
				var longDestination = (copy.Opcode >> 9) & 7;
				_buffer.WriteWord(
					copy.Offset,
					0x7000 | (longDestination << 9)); // MOVEQ #0,Dd
				_buffer.WriteWord(
					adjacentNormalization.Offset,
					0x3000 | (longDestination << 9) | longSource); // MOVE.W Ds,Dd
				_buffer.RemoveBytes(adjacentNormalization.Offset + 2, 4);
				return true;
			}
			if ((copy.Opcode & 0xF1F8) != 0x3000 ||
				copy.Length != 2 ||
				!dataflow.TryGetFacts(copy.Offset, out var copyFacts) ||
				copyFacts.LiveConditionsAfter != M68kConditionCodeSet.None)
			{
			...
			for (var index = copyIndex + 1; index < instructions.Count; index++)
			{
				...
				if (instruction.Opcode == (ushort)(0x0280 | destination) &&
					instruction.Length == 6 &&
					instruction.ExtensionLong == 0x0000FFFF &&
					(facts.LiveConditionsAfter & logicalConditions) == 0)
				{
					_buffer.RemoveBytes(instruction.Offset, instruction.Length);
					_buffer.InsertBytes(copy.Offset, 2);
				...
				var isWordRegisterShift =
					(instruction.Opcode & 0xF0C0) == 0xE040 &&
					(instruction.Opcode & 7) == destination;
				if (!isWordRegisterShift &&
					((facts.Effects.DefinesData & destinationMask) != 0 ||
					 (facts.Effects.UsesData & destinationMask) != 0))
				{
					break;
				}
	...
	private bool TryRetargetZeroExtendedWordLoad(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 3 < instructions.Count; index++)
		{
			var clearSource = instructions[index];
			var load = instructions[index + 1];
			var clearDestination = instructions[index + 2];
			var copy = instructions[index + 3];
			if ((clearSource.Opcode & 0xF1FF) != 0x7000 ||
				clearSource.Length != 2 ||
				(load.Opcode & 0xF1FF) != 0x3039 ||
				load.Length != 6 ||
				(clearDestination.Opcode & 0xF1FF) != 0x7000 ||
				clearDestination.Length != 2 ||
				(copy.Opcode & 0xF1F8) != 0x3000 ||
				copy.Length != 2)
			{
				continue;
			}

			var source = (clearSource.Opcode >> 9) & 7;
			var destination = (clearDestination.Opcode >> 9) & 7;
			if (source == destination ||
				((load.Opcode >> 9) & 7) != source ||
				(copy.Opcode & 7) != source ||
				((copy.Opcode >> 9) & 7) != destination ||
				IsReferencedLabelAt(load.Offset) ||
				IsReferencedLabelAt(clearDestination.Offset) ||
				IsReferencedLabelAt(copy.Offset) ||
				!dataflow.TryGetFacts(copy.Offset, out var copyFacts) ||
				(copyFacts.LiveDataAfter & (1 << source)) != 0)
			{
				continue;
			}

			// Both the original copy and the retargeted load establish flags from
			// the same word value, so the final condition-code state is unchanged.
			_buffer.WriteWord(
				clearSource.Offset,
				0x7000 | (destination << 9));
			_buffer.WriteWord(
				load.Offset,
				(load.Opcode & 0xF1FF) | (destination << 9));
			_buffer.RemoveBytes(clearDestination.Offset, 4);
			return true;
		}
	...
	private bool TryRemoveRedundantZeroBeforeWordMaskStore(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 4 < instructions.Count; index++)
		{
			var clear = instructions[index];
			var copy = instructions[index + 1];
			var mask = instructions[index + 2];
			var set = instructions[index + 3];
			var store = instructions[index + 4];
			var destination = clear.Opcode >> 9 & 7;
			if ((clear.Opcode & 0xF1FF) != 0x7000 || clear.Length != 2 ||
				(copy.Opcode & 0xF1F8) != 0x3000 || copy.Length != 2 ||
				((copy.Opcode >> 9) & 7) != destination ||
				(mask.Opcode & 0xFFF8) != 0x0240 || mask.Length != 4 ||
				(mask.Opcode & 7) != destination ||
				(set.Opcode & 0xFFF8) != 0x0040 || set.Length != 4 ||
				(set.Opcode & 7) != destination ||
				!dataflow.TryGetFacts(store.Offset, out var storeFacts) ||
				(storeFacts.Effects.UsesData & (1 << destination)) == 0 ||
				(storeFacts.Effects.DefinesData & (1 << destination)) != 0 ||
				store.Length < 4 ||
				instructions.Skip(index + 1).Take(4)
					.Any(instruction => IsReferencedLabelAt(instruction.Offset)))
			{
				continue;
			}

			// Every use after the word copy is word-sized. The copy overwrites the
			// clear's flags, and the upper word is never observed before the store.
		...
		IReadOnlyList<M68kEmittedInstruction> instructions)
	{
		...
		for (var index = 0; index + unmaskedPatternLength <= instructions.Count; index++)
		{
			...
			var combined = (combine.Opcode >> 9) & 7;
			var combineSource = combine.Opcode & 7;
			...
			if ((firstMove.Opcode & 0xF1F8) != 0x3000 || // MOVE.W Ds,Dt
				...
				(rightShiftThree.Opcode & 7) != temporary ||
				QuickCount(rightShiftThree.Opcode) != 3 ||
				(combine.Opcode & 0xF1F8) != 0x8040 || // OR.W Dl,Dt or Dt,Dl
				!((combineSource == savedLeft && combined == temporary) ||
				  (combineSource == temporary && combined == savedLeft)) ||
				(add.Opcode & 0xF1F8) != 0xD040 || // ADD.W Db,Dt
				((add.Opcode >> 9) & 7) != combined ||
				(copyBack.Opcode & 0xF1F8) != 0x2000 || // MOVE.L Dt,Ds
				(copyBack.Opcode & 7) != combined ||
				result == temporary ||
				result == savedLeft ||
				result == byteValue ||
				!hasResultMask &&
	...
	{
		foreach (var (label, offset) in _buffer.Labels)
		{
		...
		M68kInstructionDataflow dataflow)
	{
		...
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			_buffer.RemoveBytes(compare.Offset, compare.Length);
			_buffer.InsertBytes(moveQuick.Offset + 2, 4, shiftBoundaryLabels: true, shiftBoundaryAnchors: true);
		...
		M68kInstructionDataflow dataflow)
	{
		...
		for (var index = 0; index + 2 < instructions.Count; index++)
		{
			...
			_buffer.InsertBytes(copy.Offset + 2, 4, shiftBoundaryLabels: true, shiftBoundaryAnchors: true);
	...
	private bool TryFoldZeroMoveQuickToAddress(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
			...
			var temporary = (zero.Opcode >> 9) & 7;
			if ((moveAddress.Opcode & 7) != temporary ||
				(moveFacts.LiveDataAfter & (1 << temporary)) != 0 ||
				(moveFacts.LiveConditionsAfter &
				 M68kConditionCodeSet.All) != M68kConditionCodeSet.None)
			{
				continue;
			}
			...
			_buffer.WriteWord(
				zero.Offset,
				(ushort)(0x91C8 | (destination << 9) | destination));
			_buffer.RemoveBytes(moveAddress.Offset, moveAddress.Length);
			return true;
		}
	...
	private bool TryFoldWordConstantRegisterUse(M68kInstructionDataflow dataflow)
	{
		...
		for (var index = 0; index + 1 < instructions.Count; index++)
		{
				...
				useIndex++)
			{
				...
				var isWordOperation = logicalFamily is 0x8040 or 0xC040 or 0xB040 &&
					(use.Opcode & 7) == source;
				...
				if (isWordCopy || isWordOperation)
				{
					...
					var replacement = isWordCopy
						...
						: logicalFamily switch
						{
							...
							0xB040 => 0x0C40 | destination, // CMPI.W
	...
	private bool TryNarrowZeroExtendedRegisterAnd(
		M68kInstructionDataflow dataflow)
	{
		var instructions = dataflow.Instructions;
		for (var index = 0; index < instructions.Count; index++)
		{
			var instruction = instructions[index];
			if ((instruction.Opcode & 0xF1F8) != 0xC080 ||
				instruction.Length != 2 ||
				IsReferencedLabelAt(instruction.Offset))
			{
				continue;
			...
			var source = instruction.Opcode & 7;
			var destination = instruction.Opcode >> 9 & 7;
			var destinationRange = dataflow.GetDataValueBefore(
				instruction.Offset,
				destination);
			...
			{
				var candidate = instructions[candidateIndex];
				if (!dataflow.TryGetFacts(candidate.Offset, out var candidateFacts) ||
					candidateFacts.Effects.IsBarrier)
				{
					break;
				}
				if ((candidateFacts.Effects.DefinesData & (1 << source)) == 0)
				{
					continue;
				}
				if ((candidate.Opcode & 0xF1FF) == 0x203C && candidate.Length == 6)
				{
					mask = candidate.ExtensionLong;
					definitionIndex = candidateIndex;
				}
				else if ((candidate.Opcode & 0xF1FF) == 0x303C && candidate.Length == 4)
				{
					mask = candidate.ExtensionWord;
					definitionIndex = candidateIndex;
				}
				break;
			}
			if (definitionIndex < 0 || mask > 0x7FFF ||
				(destinationRange.KnownZeroMask & 0xFFFF0000u) != 0xFFFF0000u)
			{
				continue;
			}

			// With a zero upper destination word and a clear mask sign bit, word
			// and long AND produce the same 32-bit value and N/Z/V/C flags.
			_buffer.InsertBytes(instruction.Offset + 2, 2, shiftBoundaryLabels: true, shiftBoundaryAnchors: true);
			_buffer.WriteWord(
				instruction.Offset,
				0x0240 | destination); // ANDI.W #mask,Dn
			_buffer.WriteWord(instruction.Offset + 2, (ushort)mask);
			var definition = instructions[definitionIndex];
			if (!IsReferencedLabelAt(definition.Offset) &&
				(definitionIndex == index - 1 ||
				 dataflow.TryGetFacts(definition.Offset, out var definitionFacts) &&
				 (definitionFacts.Effects.WritesConditions &
				  definitionFacts.LiveConditionsAfter) == 0 &&
				 !IsDataRegisterReadBeforeOverwrite(
					 instructions,
					 index + 1,
					 source,
					 dataflow)))
			{
				_buffer.RemoveBytes(definition.Offset, definition.Length);
			}
			return true;
		}
	...
	private bool TryFoldTailReturn()
	{
		...
		for (var index = _buffer.Addresses.Count - 1; index >= 0; index--)
		{
			...
			if (opcodeOffset < 0 ||
				opcodeOffset + 7 >= codeEnd ||
				_buffer.ReadWord(opcodeOffset) != 0x4EB9 ||
				_buffer.ReadWord(opcodeOffset + 6) != 0x4E75 ||
				address.External ||
				HasLabelAt(opcodeOffset + 6) &&
					(address.Addend != 0 || !CanTailCallWithSharedReturn(address.Target)))
			{
				continue;
			}

			_buffer.WriteWord(opcodeOffset, 0x4EF9); // JSR -> JMP
			_buffer.InstructionEffectOverrides.Remove(opcodeOffset);
			// A conditional caller can share this RTS with a path that skipped
			// the call. Keep every labeled return entry while tail-folding the call.
			if (!HasLabelAt(opcodeOffset + 6))
			{
	...
	private bool TryRemoveBranchToNextLabel()
	{
		...
		for (var index = _buffer.Branches.Count - 1; index >= 0; index--)
		{
			...
			var length = (opcode & 0x00FF) == 0 ? 4 : 2;
			if ((opcode & 0xF000) != 0x6000 ||
				(opcode & 0xFF00) == 0x6100 ||
				branch.OpcodeOffset + length > _buffer.Bytes.Count ||
	...
	private bool TryCanonicalizeAddressAdjustments()
	{
		...
		for (var offset = startOffset; offset + 1 < endOffset; offset += 2)
		{
				...
				!HasLabelAt(offset + 2))
			{
				...
				_buffer.RemoveBytes(
					offset + replacementLength,
					first.Length - replacementLength);
				return true;
			}
		}
