...
public sealed class CompilerExecutionTests
{
	...
	public static TheoryData<M68kCpuTarget, M68kCpuModel> CpuTargets =>
		new()
		{
			...
			// Exercise MC68020 output with the exact MC68020 timing profile.
	...
	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void AllocatedLargeDenseSwitchUsesBoundedAddressTable(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		var result = Compile(
		...
		Assert.Equal(59u, ExecuteHunk(result, model));
		var dense = Assert.Single(result.Symbols, symbol =>
			symbol.Name.EndsWith("::LargeDenseSwitch", StringComparison.Ordinal));
		Assert.True(dense.Size <= 190,
			$"Large dense switch emitted {dense.Size} code bytes.");
	}

	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void PureFunctionAddressSwitchUsesValueTableOnEveryCpu(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		var result = Compile(
			target,
			M68kOutputFormat.Hunk,
			"CopperSharp.Compiler.Tests.FunctionAddressSwitchFixtures::PureEntry",
			assemblyPath: typeof(FunctionAddressSwitchFixtures).Assembly.Location);

		Assert.Equal(42u, ExecuteHunk(result, model));
		var lookup = Assert.Single(result.Symbols, symbol =>
			symbol.Name.EndsWith("::PureSwitch", StringComparison.Ordinal));
		Assert.True(lookup.Size <= 64,
			$"Pure function-address switch emitted {lookup.Size} code bytes.");
	}

	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void ImpureFunctionAddressSwitchPreservesControlDispatchFallback(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		var result = Compile(
			target,
			M68kOutputFormat.Hunk,
			"CopperSharp.Compiler.Tests.FunctionAddressSwitchFixtures::FallbackEntry",
			assemblyPath: typeof(FunctionAddressSwitchFixtures).Assembly.Location);
		...
		Assert.True(lookup.Size > 64,
			$"Impure switch was unexpectedly collapsed to {lookup.Size} code bytes.");
	}

	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void TaggedFunctionAddressSwitchUsesRelocatedValueTableOnEveryCpu(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		var result = Compile(
			target,
			M68kOutputFormat.Hunk,
			"CopperSharp.Compiler.Tests.FunctionAddressSwitchFixtures::TaggedEntry",
			assemblyPath: typeof(FunctionAddressSwitchFixtures).Assembly.Location);

		Assert.Equal(42u, ExecuteHunk(result, model));
		var lookup = Assert.Single(result.Symbols, symbol =>
			symbol.Name.EndsWith("::TaggedSwitch", StringComparison.Ordinal));
		Assert.True(lookup.Size <= 64,
			$"Tagged function-address switch emitted {lookup.Size} code bytes.");
	}

	[Fact]
	public void FunctionAddressSwitchSelectsValueTableOnlyForPureReturnArms()
	{
		var pure = Compile(
		...
		Assert.Contains(
			"allocated_002Dswitch_002Dfunction_002Daddress_002Dtable",
			pure.Text,
			StringComparison.Ordinal);
		Assert.DoesNotContain(
			"allocated_002Dswitch_002Dedge",
			pure.Text,
			StringComparison.Ordinal);
		Assert.DoesNotContain(
			"allocated_002Dswitch_002Dfunction_002Daddress_002Dtable",
			...
			StringComparison.Ordinal);
		Assert.Contains(
			"allocated_002Dswitch_002Daddress_002Dtable",
			fallback.Text,
			StringComparison.Ordinal);
	}

	[Fact]
	public void TaggedFunctionAddressValueTableCarriesLowBitAddends()
	{
		var tagged = Compile(
			M68kCpuTarget.M68000,
			M68kOutputFormat.Assembly,
		...
		Assert.Contains(
			...
			tagged.Text,
			StringComparison.Ordinal);
		Assert.Contains("+1", tagged.Text, StringComparison.Ordinal);
		Assert.DoesNotContain(
			"allocated_002Dswitch_002Dedge",
			tagged.Text,
			StringComparison.Ordinal);
	...
	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void AllocatedSparseSwitchOmitsDefaultHoles(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		var result = Compile(
			target,
			M68kOutputFormat.Hunk,
			"CopperSharp.Compiler.Tests.CompilerFixtures::AllocatedSparseSwitchEntry");

		Assert.Equal(42u, ExecuteHunk(result, model));
		if (target == M68kCpuTarget.M68000)
		{
			var sparse = Assert.Single(result.Symbols, symbol =>
		...
		M68kCpuModel model)
	{
		...
		if (target == M68kCpuTarget.M68000)
		{
			...
			Assert.Equal(3, result.AllocationStatistics.Count);
			Assert.All(
	...
	public void PortableStopwatchTimestampFastPathHasPinnedMc68000Cost()
	{
		const ulong ticks = 0x1122_3344_5566_7788UL;
		var result = Compile(
			M68kCpuTarget.M68000,
		...
		Assert.Equal(1, probe.Reads);
		Assert.Equal(1, probe.Closes);
		Assert.Equal(2_360, cycles);
		Assert.Equal(205, instructions);
	}

	[Fact]
	[Trait("Category", "Benchmark")]
	public void StopwatchBenchmarkCrcLoopHasPinnedMc68000Cost()
	{
		var result = AmigaM68kCompiler.Compile(new M68kCompilationRequest
		{
			AssemblyPath = typeof(StopwatchBenchmarkExample.Program).Assembly.Location,
			EntryPoint = "StopwatchBenchmarkExample.Program::ComputeCrc32",
		...
		Assert.Equal(0x003A_981Du, checksum);
		Assert.Equal(458_884, cycles);
		Assert.Equal(58_892, instructions);
	}
	...
	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void FixedPointMatchesDisabledExecutionAndFinalConditionCodes(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		var entries = new[]
		{
			...
			"CopperSharp.Compiler.Tests.CompilerFixtures::VariableShiftDifferentialEntry",
		...
		foreach (var entry in entries)
		{
			...
			var fixedPoint = Compile(
				target,
				M68kOutputFormat.Hunk,
				entry,
				peepholeOptimization: M68kPeepholeOptimizationMode.FixedPoint);
			(uint Result, ushort ConditionCodes) expected;
			(uint Result, ushort ConditionCodes) actual;
			try
			{
				expected = ExecuteWithConditionCodes(disabled, model);
				actual = ExecuteWithConditionCodes(fixedPoint, model);
			}
			catch (Exception exception)
			{
				throw new InvalidOperationException(
					$"Optimizer differential failed for {target} {entry}.",
					exception);
			}
	...
	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void PointerWrapperInternalAbiUsesExistingAddressSlots(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		const string entry =
			"CopperSharp.Compiler.Tests.CompilerFixtures::PointerWrapperInternalAbiEntry";
		var result = Compile(target, M68kOutputFormat.Hunk, entry);

		Assert.Equal(42u, ExecuteHunk(result, model));
		Assert.DoesNotContain("stack-bytes=", result.Map, StringComparison.Ordinal);
	}

	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void InternalAbiUsesFreeAddressSlotForFinalScalar(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		const string entry =
			"CopperSharp.Compiler.Tests.CompilerFixtures::MixedBankInstanceAbiEntry";
		var result = Compile(target, M68kOutputFormat.Hunk, entry);
	...
	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void InternalAbiMaterializesNarrowAddressTransportInDataBank(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		const string entry =
			"CopperSharp.Compiler.Tests.CompilerFixtures::MixedBankNarrowTransportAbiEntry";
		var result = Compile(target, M68kOutputFormat.Hunk, entry);
	...
	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void BptrConversionsUseADataRegisterForTheirShifts(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		const string entry =
			"CopperSharp.Compiler.Tests.CompilerFixtures::BptrAddressRoundTripEntry";
		var result = Compile(target, M68kOutputFormat.Hunk, entry);
	...
	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void RelaxedStackLeaForwardingPreservesStackPushExecution(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		static M68kAssembler CreateAssembler()
		{
			var assembler = new M68kAssembler();
			assembler.Mark("method:entry");
			assembler.Mark("method:entry:BB0000");
			...
			assembler.Mark("method:entry:end");
			return assembler;
		}

		var unoptimized = CreateAssembler().Link(HunkLoadAddress,
			new Dictionary<string, uint>());
		var optimizedAssembler = CreateAssembler();
		optimizedAssembler.OptimizeForCpu(target);
		var optimized = optimizedAssembler.Link(HunkLoadAddress,
			new Dictionary<string, uint>());

		static (uint Result, ushort Conditions, uint Stack) Run(
			LinkedCode linked,
			M68kCpuModel cpuModel)
		{
			var bus = new TestBus();
			linked.Bytes.CopyTo(bus.Memory.AsSpan((int)HunkLoadAddress));
			bus.WriteLong(StackPointer + 8, 0x8123_4567);
			ushort conditions = 0;
			uint stack = 0;
			var result = Execute(bus, cpuModel, HunkLoadAddress,
				afterReturn: state =>
				{
					conditions = (ushort)(state.StatusRegister & 0x001F);
					stack = state.A[7];
				});
			return (result, conditions, stack);
		}

		var expected = Run(unoptimized, model);
		var actual = Run(optimized, model);
		Assert.Equal((0x8123_4567u, expected.Conditions, StackPointer + 4), expected);
		Assert.Equal(expected, actual);
	}

	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void RelaxedStackLeaForwardingPreservesMemoryStoreExecution(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		static M68kAssembler CreateAssembler()
		{
			...
			assembler.Mark("method:entry");
			assembler.Mark("method:entry:BB0000");
			assembler.EmitWord(0x3A7C); // MOVEA.W #$3000,A5
			assembler.EmitWord(0x3000);
			assembler.EmitWord(0x41D7); // LEA (A7),A0
			assembler.EmitWord(0x224D); // MOVEA.L A5,A1
			assembler.EmitWord(0x2229); // MOVE.L 4(A1),D1
			assembler.EmitWord(4);
			assembler.EmitWord(0x2141); // MOVE.L D1,8(A0)
			assembler.EmitWord(8);
			assembler.EmitWord(0x202F); // MOVE.L 8(A7),D0
			assembler.EmitWord(8);
			assembler.EmitWord(0x4E75);
			assembler.Mark("method:entry:end");
			return assembler;
			...
			M68kCpuModel cpuModel)
		{
			var bus = new TestBus();
			linked.Bytes.CopyTo(bus.Memory.AsSpan((int)HunkLoadAddress));
			bus.WriteLong(0x0000_3004, 0x8123_4567);
			ushort conditions = 0;
			uint stack = 0;
			var result = Execute(bus, cpuModel, HunkLoadAddress,
				afterReturn: state =>
	...
	[InlineData(M68kCpuTarget.M68040, M68kCpuModel.M68040)]
	public void BasePlusIndexFoldPreservesStoreExecutionAndConditions(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		static M68kAssembler CreateAssembler()
		...
		static (uint Result, uint Stored, ushort Conditions, uint Stack) Run(
			LinkedCode linked,
			M68kCpuModel cpuModel)
		{
			var bus = new TestBus();
			...
			return (result, bus.ReadLong(0x0000_3004), conditions, stack);
		}

		var expected = Run(unoptimized, model);
		var actual = Run(optimized, model);
		Assert.Equal((uint.MaxValue, uint.MaxValue, (ushort)0x0008,
			StackPointer + 4), expected);
		Assert.Equal(expected, actual);
		Assert.Contains("move.l\td0,(a5,d3.l)", optimizedAssembly,
			StringComparison.Ordinal);
		Assert.DoesNotContain("adda.l\td3,a0", optimizedAssembly,
			StringComparison.Ordinal);
	}

	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void RtsLeafCallEliminationPreservesExecution(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		static M68kAssembler CreateAssembler()
		{
			...
			assembler.EmitWord(0xD07C); // ADD.W #23,D0
			assembler.EmitWord(23);
			assembler.EmitWord(0x4E75);
			assembler.Mark("method:entry:end");
		...
		var expected = Run(unoptimized, model);
		var actual = Run(optimized, model);
		Assert.Equal((42u, expected.Conditions, StackPointer + 4), expected);
		Assert.Equal(expected, actual);
	}
	...
	public void CompilationReportsFinalLinkedLoopFootprints()
	{
		...
		Assert.Contains("MACHINE scc=", result.Map, StringComparison.Ordinal);
		Assert.Contains("devirtualized-calls=", result.Map, StringComparison.Ordinal);
		Assert.Contains("inlined-calls=", result.Map, StringComparison.Ordinal);
		Assert.Contains(
	...
	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void AggressiveGuestMemoryWrappersInlineAndPreserveExecution(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		var result = Compile(
			...
			"CopperSharp.Compiler.Tests.CompilerFixtures::" +
				"AggressiveGuestWriteReadEntry");

		Assert.Equal(0x1122_3344u, ExecuteHunk(result, model));
		Assert.DoesNotContain(
			"CompilerFixtures::AggressiveGuestRead ",
			result.Map,
			StringComparison.Ordinal);
		Assert.DoesNotContain(
			"CompilerFixtures::AggressiveGuestWrite ",
			result.Map,
			StringComparison.Ordinal);
	}
	...
	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void AggressiveConstrainedGuestMemoryInliningPreservesForwardedArgument(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		var result = Compile(
			...
			M68kOutputFormat.Hunk,
			"CopperSharp.Compiler.Tests.CompilerFixtures::" +
				"AggressiveConstrainedGuestPassThroughEntry",
			peepholeOptimization: M68kPeepholeOptimizationMode.Disabled);

		Assert.Equal(0x99AA_BBCCu, ExecuteHunk(result, model));
		Assert.DoesNotContain(
			"AggressiveGuestMemoryFixture::Read ",
			result.Map,
			StringComparison.Ordinal);
		Assert.DoesNotContain(
			"AggressiveGuestMemoryFixture::Write ",
			result.Map,
			StringComparison.Ordinal);
	}
	...
	public void HybridDirectCallAssemblyUsesRegistersWithoutShadowSlots()
	{
		...
		Assert.DoesNotContain("\tmove.l\td1,-(a7)", result.Text, StringComparison.Ordinal);
		Assert.Contains("\tmovea.w\t#$0003,a1", result.Text, StringComparison.Ordinal);
	...
	[Fact]
	public void SingleWordStackArgumentHomePreservesCallerLiveRegistersOnMc68000()
	{
		const string entry =
			"CopperSharp.Compiler.Tests.CompilerFixtures::" +
		...
		Assert.Matches("\\tmove\\.l\\t[0-9]+\\(a7\\),d[0-7]", assembly.Text!);
	}

	[Fact]
	public void SingleWordStackArgumentHomeUsesAnchoredDestinationAcrossScratchUse()
	{
		const string entry =
			"CopperSharp.Compiler.Tests.CompilerFixtures::" +
		...
		Assert.Matches(
			"\\tmove\\.l\\t[0-9]+\\(a7\\),[0-9]+\\(a5\\)",
			assembly.Text!);
		Assert.Contains("\taddq.l\t#4,a7", assembly.Text, StringComparison.Ordinal);
	}

	[Fact]
	public void MultiwordStackArgumentHomeCopiesDirectlyToAnchoredDestination()
	{
		const string entry =
			"CopperSharp.Compiler.Tests.CompilerFixtures::" +
		...
		Assert.Equal(42u, ExecuteHunk(result, M68kCpuModel.M68000));
		Assert.Contains("\tmovea.l\ta7,a5", assembly.Text, StringComparison.Ordinal);
		// The direct memory copy leaves SP unchanged, so incoming arguments do
		// not include the former four-byte D7 scratch-preservation adjustment.
		Assert.DoesNotContain("\tmove.l\td7,-(a7)", assembly.Text, StringComparison.Ordinal);
		Assert.Contains("\tmove.l\t60(a7),36(a5)", assembly.Text, StringComparison.Ordinal);
		Assert.Contains("\tmove.l\t64(a7),40(a5)", assembly.Text, StringComparison.Ordinal);
		Assert.DoesNotContain("\tmove.l\td7,36(a5)", assembly.Text, StringComparison.Ordinal);
		Assert.DoesNotContain("\tmove.l\t72(a7),44(a5)", assembly.Text, StringComparison.Ordinal);
		Assert.DoesNotContain("\tmove.l\t(a7)+,d7", assembly.Text, StringComparison.Ordinal);
	}
	...
	[Fact]
	public void InlinedCrossModuleIntrinsicResolvesInItsSourceMethod()
	{
		var result = AmigaM68kCompiler.Compile(new M68kCompilationRequest
		{
			AssemblyPath = FixtureAssembly,
			ManagedAssemblyPaths =
			[
				typeof(CopperSharp.Compiler.Tests.MultiModule.ExternalMethods)
					.Assembly.Location
			],
			EntryPoint = "CopperSharp.Compiler.Tests.CompilerFixtures::" +
				"MultiModuleAggressiveGuestReadEntry",
			Cpu = M68kCpuTarget.M68000,
			OutputFormat = M68kOutputFormat.Hunk
		});

		Assert.Equal(0xCAFE_BABEu, ExecuteHunk(result, M68kCpuModel.M68000));
		Assert.DoesNotContain(
			result.Symbols,
			symbol => symbol.Name.EndsWith(
				"::ReadGuestUInt32",
				StringComparison.Ordinal));
	}
	...
	public void FullExceptionModeOmitsHandlerHelpersWhenNoHandlerCanUseThem()
	{
		...
		Assert.DoesNotContain(
			"__c68k_exception_type_match:",
			result.Text,
			StringComparison.Ordinal);
		Assert.DoesNotContain(
			"__c68k_exception_endfinally:",
			result.Text,
			StringComparison.Ordinal);
	}

	[Fact]
	public void YoloFreestandingAssemblyOmitsUnusedUnwindSiteLabels()
	{
		var result = AmigaM68kCompiler.Compile(new M68kCompilationRequest
		{
			AssemblyPath = FixtureAssembly,
		...
		Assert.DoesNotContain(
			"C68K_generated_003Aunwind_site",
			result.Text,
			StringComparison.Ordinal);
	}
	...
	public void FullExceptionModeRecordsTypedCatchClauses()
	{
		...
		Assert.Contains("C68K_runtime_003Aexception_002Dtable", result.Text, StringComparison.Ordinal);
		Assert.Contains("__c68k_exception_type_match:", result.Text, StringComparison.Ordinal);
		Assert.DoesNotContain("__c68k_eh_", result.Text, StringComparison.Ordinal);
	}
	...
	public void AmigaUnhandledExceptionUsesRequesterAfterOptionalHook()
	{
		...
		Assert.Contains(
			...
			withHook.Text,
			StringComparison.Ordinal);
		var hookText = withHook.Text!;
		var hookCall = hookText.IndexOf(
			$"\tjsr\t{M68kRuntimeImports.UnhandledException}",
			StringComparison.Ordinal);
		var requester = hookText.IndexOf(
			"__c68k_amiga_unhandled_requester:",
			StringComparison.Ordinal);
		Assert.True(hookCall >= 0 && requester > hookCall);
		Assert.DoesNotContain("\tillegal", hookText[hookCall..requester], StringComparison.Ordinal);
		Assert.Contains("\tillegal", hookText, StringComparison.Ordinal);
	}
		...
		M68kCpuModel model)
	{
		...
		Assert.Equal(
			...
			Regex.Matches(
				result.Text!,
				@"\tmovea\.l\t_ExecBase(?:\(pc\))?,a6").Count);
		Assert.DoesNotMatch(
			@"\tmove\.l\t_ExecBase(?:\(pc\))?,(?<base>d[2-7])\r?\n" +
			@"\tmovea\.l\t\k<base>,a6",
			result.Text!);
		Assert.Matches(
		...
		M68kCpuModel model)
	{
		...
		Assert.Contains("\tmovea.l\t_ExecBase,a6", result.Text, StringComparison.Ordinal);
	}
		...
		M68kCpuModel model)
	{
		...
		Assert.Equal(
			...
			Regex.Matches(
				...
				@"\tmovea\.l\t_ExecBase(?:\(pc\))?,a6").Count);
		Assert.DoesNotMatch(
			@"\tmovea?\.l\t_ExecBase(?:\(pc\))?,(?<base>[ad][0-5])\r?\n\tmovea\.l\t\k<base>,a6",
			text);
		Assert.Matches(
	...
	{
		const uint execBase = 0x0000_3600;
		const uint incomingA6 = 0x005A_6006;
		var result = AmigaM68kCompiler.Compile(new M68kCompilationRequest
		{
			AssemblyPath = FixtureAssembly,
			EntryPoint =
		...
		var helper = Assert.Single(result.Symbols, static symbol =>
			symbol.Name.EndsWith("::PlatformBaseIncomingA6Helper", StringComparison.Ordinal));
		var helperAddress = HunkLoadAddress + helper.Address;
		uint? helperReturnAddress = null;
		uint helperStack = 0;
		var helperEntries = 0;
		var helperReturns = 0;
		var bus = CreateHunkBus(result);
		...
		Assert.Equal(
			...
			Execute(
				...
				beforeInstruction: (cpu, memory) =>
				{
					if (cpu.State.ProgramCounter == helperAddress)
					{
						Assert.Null(helperReturnAddress);
						helperEntries++;
						helperStack = cpu.State.A[7];
						helperReturnAddress = memory.ReadLong(helperStack);
						// A6 is callee-saved, not an argument. A distinct incoming
						// value proves both the library reload and its restoration.
						cpu.State.A[6] = incomingA6;
					}
					else if (cpu.State.ProgramCounter == helperReturnAddress)
					{
						Assert.Equal(incomingA6, cpu.State.A[6]);
						Assert.Equal(helperStack + 4u, cpu.State.A[7]);
						helperReturnAddress = null;
						helperReturns++;
					}
				}));
		Assert.Equal(1, typeOfMemCalls);
		Assert.Equal(1, freeMemCalls);
		Assert.Equal(1, helperEntries);
		Assert.Equal(1, helperReturns);
		Assert.Null(helperReturnAddress);
		Assert.Matches(
	...
	public void CompilesMuiSunflowerSampleEventLoop()
	{
		...
		Assert.DoesNotMatch(
			@"\tmove\.l\t\d+\(a7\),-\(a7\)\r?\n(?:[A-Za-z0-9_:]+:\r?\n)*\tmove\.l\t\(a7\)\+,d0",
			result.Text);
		Assert.Matches(@"\tmove\.l\td0,\(a[0-6]\)", result.Text);
		Assert.DoesNotContain(
			"\tdc.w\t$486F",
			result.Text,
			StringComparison.Ordinal);
	...
	public void CompilesIffInspectSampleWithTypedExceptionsAndCleanup()
	{
		...
		Assert.DoesNotContain(
			...
			StringComparison.Ordinal);
		Assert.Contains(
			"\tmovea.l\t_IFFParseLibraryBase,a1",
			result.Text,
			StringComparison.Ordinal);
		Assert.Contains(
			"\tmovea.l\t_DOSLibraryBase,a1",
			result.Text,
			StringComparison.Ordinal);
		Assert.Contains(
		...
		Assert.DoesNotMatch(
			...
			result.Text);
		Assert.Matches(
			@"\taddq\.l\t#1,C68K_static_003ACopperSharp_002ERuntime_002EManaged_003A[0-9A-F]+",
			result.Text);
		Assert.Matches(
		...
		Assert.Matches(
			...
			@"\tmovea\.l\t\k<cursor>,(?<address>a[0-6])\r?\n" +
	...
	public void DosSampleUsesBoundedLoopForFileInfoBlockInitialization()
	{
		...
		Assert.Matches(
			@"\tmoveq\t#64,d(?<count>[0-7])\r?\n" +
			@"(?<loop>[^:]+):\r?\n" +
			@"\tmove\.l\td[0-7],\(a[0-6]\)\+\r?\n" +
			@"\tdbra\td\k<count>,\k<loop>",
			result.Text!);
		Assert.DoesNotMatch(
	...
	[Fact]
	[Trait("Category", "Benchmark")]
	public void ConsoleIoInputAndEofPathsHavePinnedMc68000Costs()
	{
		ExecutionMeasurement input = default!;
		var inputOutput = ExecuteConsoleIoSample(
			M68kCpuTarget.M68000,
			M68kCpuModel.M68000,
			[(byte)'A', (byte)'\n'],
			0,
		...
		Assert.Equal(
			"Console input/output example\nType text: A\nCharacters read: 1\n",
			System.Text.Encoding.Latin1.GetString(inputOutput));

		ExecutionMeasurement eof = default!;
		var eofOutput = ExecuteConsoleIoSample(
			M68kCpuTarget.M68000,
			M68kCpuModel.M68000,
			[],
			5,
			2,
			measurement => eof = measurement);
		Assert.Equal(
			"Console input/output example\nType text: ",
			System.Text.Encoding.Latin1.GetString(eofOutput));
		Assert.Equal(30_132, input.Cycles);
		Assert.Equal(3_112, input.Instructions);
		Assert.Equal(16_606, eof.Cycles);
		Assert.Equal(1_797, eof.Instructions);
	}
		...
		M68kCpuModel model)
	{
		...
		Assert.Equal(2, calls);
		var text = result.Text!;
		var baseLoads = Regex.Matches(
			text,
			@"\tmovea?\.l\t_ExecBase,(?<base>[ad][0-7])");
		Assert.Equal(2, baseLoads.Count);
		var baseRegister = baseLoads[0].Groups["base"].Value;
		Assert.All(
			baseLoads.Cast<Match>(),
			match => Assert.Equal(baseRegister, match.Groups["base"].Value));
		Assert.Equal(
			3,
			Regex.Matches(
				text,
				$@"\tmovea\.l\t{Regex.Escape(baseRegister)},a6").Count);
		Assert.DoesNotContain(
			"\tmovea.l\t_ExecBase,a6",
			text,
			StringComparison.Ordinal);
	...
	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void AmigaIndirectSubroutineUsesSuppliedAddressAndRegisterAbi(
		M68kCpuTarget target, M68kCpuModel model)
	{
		const uint entry = 0x0000_3500;
		const uint context = 0x0000_3600;
		var result = AmigaM68kCompiler.Compile(new M68kCompilationRequest
		{
			AssemblyPath = FixtureAssembly,
			EntryPoint =
				"CopperSharp.Compiler.Tests.CompilerFixtures::CallAmigaIndirectSubroutine",
			Cpu = target,
			OutputFormat = M68kOutputFormat.Assembly
		});
		var bus = CreateHunkBus(result);
		bus.RegisterGateway(entry, state =>
		{
			Assert.Equal(entry, state.A[3]);
			Assert.Equal(context, state.A[6]);
			state.D[0] += state.D[1];
		});
		...
		Assert.Equal(
			42u,
			Execute(bus, model, HunkLoadAddress + result.EntryPoint));
		Assert.Matches(@"\tj(?:sr|mp)\t(?:0)?\(a3\)", result.Text);
	}
	...
	public void FileStatsDefersWordNormalizationUntilWideningBoundary()
	{
		...
		Assert.Equal(
			...
			Regex.Matches(
				result.Text!,
				@"\tmoveq\t#0,d([0-7])\r?\n\tmove\.w\td[0-7],d\1").Count);
		Assert.Single(
			Regex.Matches(
				result.Text!,
				@"\tandi\.l\t#\$000000FF,d([0-7])\r?\n\tmovea\.l\td\1,a[01]"));
		Assert.DoesNotMatch(
		...
		Assert.Equal(4, printReportStatistics.CalleeSavedRegisters);
		...
		Assert.Contains(
			...
			StringComparison.Ordinal);
		Assert.DoesNotMatch(
			@"\tmove\.l\t_DOSLibraryBase,d([0-7])\r?\n\tmovea\.l\td\1,a1",
			result.Text);
		Assert.Matches(
			@"\tmove\.l\t#C68K_cstring_[^,\r\n]+,d1\r?\n" +
			@"\tmovea\.l\t_DOSLibraryBase,a6\r?\n" +
			@"\tmove\.l\ta7,d2\r?\n" +
			@"\tjsr\t-954\(a6\)",
			result.Text);
		Assert.Contains(
		...
		var loopMatch = Regex.Match(
			result.Text!,
			@"(?<header>C68K_method_003A06000002_003ABB[0-9A-F]+):\r?\n" +
			@"\tmove\.l\ta2,d1\r?\n" +
			@"\tmovea\.l\t\(a7\),a6\r?\n" +
			@"\tjsr\t-306\(a6\)\r?\n" +
			@"(?<body>.*?)\tbra(?:\.[sw])?\t\k<header>",
			RegexOptions.Singleline);
	...
	[Fact]
	public void KickstartRomReadsExecBaseDirectlyWithoutWritableBaseStorage()
	{
		var result = AmigaM68kCompiler.Compile(new M68kCompilationRequest
		{
			AssemblyPath = FixtureAssembly,
		...
		Assert.DoesNotContain(result.Symbols, symbol => symbol.Name == "_ExecBase");
		Assert.Contains("initialized-ram-bytes=0 bss-bytes=0", result.Map,
			StringComparison.Ordinal);
	}

	[Fact]
	public void RomProfileAssemblyReadsExecBaseDirectlyWithoutWritableBaseStorage()
	{
		var result = AmigaM68kCompiler.Compile(new M68kCompilationRequest
		{
			AssemblyPath = FixtureAssembly,
		...
		Assert.Equal(
			...
			result.Text!.Split(
				...
				StringSplitOptions.None).Length - 1);
		Assert.DoesNotMatch(
			@"\tmovea\.l\t\$0004\.w,(?<base>a[0-5])\r?\n\tmovea\.l\t\k<base>,a6",
			result.Text);
		Assert.DoesNotContain("_ExecBase", result.Text,
			StringComparison.Ordinal);
		Assert.Contains("initialized-ram-bytes=0 bss-bytes=0", result.Map,
			StringComparison.Ordinal);
	}
	...
	[Fact]
	public void ZeroManagedFieldsArePromotedWithoutRemovingConstruction()
	{
		var result = M68kCompiler.Compile(new M68kCompilationRequest
		{
			AssemblyPath = FixtureAssembly,
		...
		foreach (var assembly in new[] { result.Text!, m68020.Text!, m68020Always.Text! })
		{
			var end = assembly.IndexOf("_003Aend:", StringComparison.Ordinal);
			Assert.True(end > 0);
			var entryBody = assembly[..end];
			Assert.Contains("__c68k_alloc", entryBody, StringComparison.Ordinal);
			Assert.Contains("\tbsr", entryBody, StringComparison.Ordinal);
			Assert.DoesNotMatch(
				@"\t(?:move|clr)\.l\t[^\r\n]*12\(a[0-6]\)",
				entryBody);
		}
	...
	public void MemoryCopyAssemblyUsesSharedInlineMemmoveKernel()
	{
		...
		Assert.Contains("\tmove.l\t(a2),(a1)", assembly.Text, StringComparison.Ordinal);
		Assert.DoesNotContain("\tmove.l\t(a2),d0", assembly.Text, StringComparison.Ordinal);
	}
	...
	[Fact]
	public void PromotesReferenceFreeAggregateHeapLanesWithoutRemovingConstruction()
	{
		M68kCompilationResult CompileAggregate(string entry) =>
			M68kCompiler.Compile(new M68kCompilationRequest
			{
				AssemblyPath = FixtureAssembly,
				EntryPoint =
		...
		static string EntryBody(M68kCompilationResult result)
		{
		...
		var field = CompileAggregate("MultiwordInstanceFieldEntry");
		var array = CompileAggregate("ThreeWordArrayEntry");
		Assert.Equal(
			42u,
			ExecuteHunk(field, M68kCpuModel.M68000));
		Assert.Equal(
			42u,
			ExecuteHunk(array, M68kCpuModel.M68000));

		var fieldBody = EntryBody(field);
		Assert.Contains("__c68k_alloc", fieldBody, StringComparison.Ordinal);
		Assert.Contains("\tbsr", fieldBody, StringComparison.Ordinal);
		Assert.DoesNotMatch(
			@"\t(?:move|clr)\.l\t[^\r\n]*(?:8|12)\(a[0-6]\)",
			fieldBody);

		var arrayBody = EntryBody(array);
		Assert.Contains("__c68k_alloc", arrayBody, StringComparison.Ordinal);
		Assert.Contains("\tbsr", arrayBody, StringComparison.Ordinal);
		Assert.DoesNotMatch(
			@"\t(?:move|clr)\.l\t[^\r\n]*(?:12|16|20)\(a[0-6]\)",
			arrayBody);
	}
		...
		M68kCpuModel model)
	{
		...
		if (target == M68kCpuTarget.M68000)
		{
			...
			Assert.Equal(14, result.AllocationStatistics.Count);
			var maximumSpillFrameBytes = result.AllocationStatistics.Max(
				static item => item.SpillFrameBytes);
			Assert.True(
				maximumSpillFrameBytes <= 16,
				$"File.SetAttributes spill frame grew to {maximumSpillFrameBytes} bytes.");
		}
	...
	public void PortableConsoleInt64MetricsStayWithinMc68000Budgets()
	{
		...
		Assert.InRange(measurement.MaximumSpillBytes, 0, 4);
	}
	...
	[Fact]
	public void ConstantIndexedArrayTrafficIsPromotedButAllocationIsRetained()
	{
		var result = M68kCompiler.Compile(new M68kCompilationRequest
		{
			AssemblyPath = FixtureAssembly,
		...
		var methodStart = result.Text!.IndexOf(
			"C68K_method_",
			StringComparison.Ordinal);
		var methodEnd = result.Text.IndexOf(
			"_003Aend:",
			methodStart,
			StringComparison.Ordinal);
		var body = result.Text[methodStart..methodEnd];

		Assert.Contains("jsr\t__c68k_alloc", body, StringComparison.Ordinal);
		Assert.Contains("C68K_array_003Asbyte", body, StringComparison.Ordinal);
		Assert.DoesNotMatch(
			@"\t(?:move|clr)\.b\t[^\r\n]*12\(a[0-6]",
			body);
	}
	...
	public void M68000SignExtendsBytesWithTwoInstructions()
	{
		var result = Compile(
			M68kCpuTarget.M68000,
			M68kOutputFormat.Assembly,
			"CopperSharp.Compiler.Tests.CompilerFixtures::DynamicSignedByteArrayEntry");
	...
	public void M68020AndLaterUseExtbLong(M68kCpuTarget target)
	{
		...
		Assert.Matches(@"\textb\.l\td[0-7]", result.Text);
		Assert.DoesNotMatch(
			@"\text\.w\td([0-7])\r?\n\text\.l\td\1",
			result.Text);
	}
	...
	[Fact]
	public void EmitsNarrowNormalizationAfterPromotedStoresAndAtAbiBoundaries()
	{
		...
		Assert.Matches(
			@"\tadd(?:q)?\.b\t(?:#[1-8]|d[0-7]),d[0-7]\r?\n\tmoveq\t#0,d([0-7])\r?\n\tmove\.b\td[0-7],d\1",
			arrays);
		Assert.Matches(
			@"\tadd(?:q)?\.w\t(?:#[1-8]|d[0-7]),d[0-7]\r?\n\tmoveq\t#0,d([0-7])\r?\n\tmove\.w\td[0-7],d\1",
			arrays);
		Assert.True(Count(arrays, @"\tandi\.l\t#\$000000FF,d[0-7]") == 0);
		Assert.True(Count(arrays, @"\tandi\.l\t#\$0000FFFF,d[0-7]") == 0);
		...
		Assert.Matches(@"\tcmp(?:i)?\.w", compare);
		Assert.Contains("cmp.b", compare, StringComparison.Ordinal);
		Assert.DoesNotContain("eori.w", compare, StringComparison.Ordinal);
	...
	public void EmitsCpuSpecificIndexedArrayEffectiveAddresses()
	{
		var m68000 = Compile(
			M68kCpuTarget.M68000,
			M68kOutputFormat.Assembly,
			"CopperSharp.Compiler.Tests.CompilerFixtures::DynamicShortArrayEntry");
		var m68020 = Compile(
			M68kCpuTarget.M68020,
			M68kOutputFormat.Assembly,
			"CopperSharp.Compiler.Tests.CompilerFixtures::DynamicShortArrayEntry");
	...
	[Theory]
	[MemberData(nameof(CpuTargets))]
	public void CanonicalizesFoldedAptrUnsignedWordResultsBeforeLongArithmetic(
		M68kCpuTarget target,
		M68kCpuModel model)
	{
		var result = Compile(
			target,
			M68kOutputFormat.Hunk,
			"CopperSharp.Compiler.Tests.CompilerFixtures::AptrUnsignedWordPromotionEntry",
			peepholeOptimization: M68kPeepholeOptimizationMode.Bounded);
		var seeded = false;
		var actual = ExecuteHunk(result, model, (cpu, _) =>
		{
			if (seeded) return;
			seeded = true;
			for (var register = 0; register < 8; register++)
				cpu.State.D[register] = 0xA5A5_0000u | (uint)register;
		});

		Assert.Equal(0x05E0u, actual);
	}
	...
	[Theory]
	[InlineData("PromotedReferenceFieldAcrossCollectionEntry")]
	[InlineData("PromotedReferenceArrayAcrossCollectionEntry")]
	public void PromotedReferenceStorageRetainsValueAndOwnerAcrossCollection(
		string entry)
	{
		var result = M68kCompiler.Compile(new M68kCompilationRequest
		{
		...
		Assert.Equal(42u, ExecuteHunk(result, M68kCpuModel.M68000));
		if (entry.Contains("Array", StringComparison.Ordinal))
		{
			var start = result.Text!.IndexOf(
				"C68K_method_",
				StringComparison.Ordinal);
	...
	public void AssemblyOutputPreservesLabelsWordsAndSymbolicImports()
	{
		...
		Assert.Contains("\tsection\trom_code,code", result.Text, StringComparison.Ordinal);
	...
	[Fact]
	public void KeepsCallerFrameAliveWhenTailCallArgumentAliasesLocalStruct()
	{
		var result = Compile(
			M68kCpuTarget.M68000,
			M68kOutputFormat.Assembly,
		...
		Assert.Matches(@"\tbsr\.[sw]\tC68K_method_", methodBody);
		Assert.DoesNotMatch(
		...
		M68kCpuModel model)
	{
		...
		Assert.Contains("\tlea\t12(a7),a7", assembly.Text, StringComparison.Ordinal);
	}
	...
	private sealed record ExecutionMeasurement(long Cycles, int Instructions);
	...
	private static byte[] ExecuteConsoleIoSample(
		...
		Action<ExecutionMeasurement>? measure = null)
	{
		const uint execBase = 0x0000_3000;
		const uint dosBase = 0x0000_5000;
		const uint inputHandle = 0x0000_0123;
		...
		Assert.Equal(
			...
			bus.ReadLong(HunkLoadAddress + dosBaseSlot.Address));
		Assert.DoesNotContain(M68kRuntimeImports.Allocate, result.Map);
		measure?.Invoke(new ExecutionMeasurement(cycles, instructions));
	...
	private static (uint Result, ushort ConditionCodes) ExecuteWithConditionCodes(
		M68kCompilationResult result,
		M68kCpuModel model)
	{
		var bus = CreateHunkBus(result);
		ushort conditionCodes = 0;
		...
		try
		{
			value = Execute(
				...
				beforeInstruction: (cpu, memory) =>
				{
					...
					trace.Enqueue(
						$"${cpu.State.ProgramCounter:X8}:${memory.ReadWord(cpu.State.ProgramCounter):X4}");
				},
				afterReturn: state => conditionCodes =
					(ushort)(state.StatusRegister & 0x001F));
		}
		catch (Exception exception)
		{
			throw new InvalidOperationException(
				$"Execution trace: {string.Join(", ", trace)}",
				exception);
		}
		return (value, conditionCodes);
	}
